...
int registry_init(void) {
    ...
    if(web_server_mode != WEB_SERVER_MODE_NONE) {
        ...
        info("Registry is disabled - use the central hibenchmarks");
        config_set_boolean(CONFIG_SECTION_REGISTRY, "enabled", 0);
        registry.enabled = 0;
    }

    // pathnames
    snprintfz(filename, FILENAME_MAX, "%s/registry", hibenchmarks_configured_varlib_dir);
    ...
    // filenames
    snprintfz(filename, FILENAME_MAX, "%s/hibenchmarks.public.unique.id", registry.pathname);
    registry.machine_guid_filename = config_get(CONFIG_SECTION_REGISTRY, "hibenchmarks unique id file", filename);
    ...
    registry.registry_to_announce = config_get(CONFIG_SECTION_REGISTRY, "registry to announce", "https://registry.my-hibenchmarks.io");
    registry.hostname = config_get(CONFIG_SECTION_REGISTRY, "registry hostname", hibenchmarks_configured_hostname);
    registry.verify_cookies_redirects = config_get_boolean(CONFIG_SECTION_REGISTRY, "verify browser cookies support", 1);

    setenv("HIBENCHMARKS_REGISTRY_HOSTNAME", registry.hostname, 1);
    setenv("HIBENCHMARKS_REGISTRY_URL", registry.registry_to_announce, 1);
    ...
    // initialize locks
    hibenchmarks_mutex_init(&registry.lock);

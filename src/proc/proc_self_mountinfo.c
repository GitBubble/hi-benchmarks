...
struct mountinfo *mountinfo_read(int do_statvfs) {
    ...
    snprintfz(filename, FILENAME_MAX, "%s/proc/self/mountinfo", hibenchmarks_configured_host_prefix);
    procfile *ff = procfile_open(filename, " \t", PROCFILE_FLAG_DEFAULT);
    if(unlikely(!ff)) {
        snprintfz(filename, FILENAME_MAX, "%s/proc/1/mountinfo", hibenchmarks_configured_host_prefix);
    ...
    for(l = 0; l < lines ;l++) {
        ...
        *minor = '\0';
        ...
        hibenchmarks_fix_chart_id(mi->persistent_id);

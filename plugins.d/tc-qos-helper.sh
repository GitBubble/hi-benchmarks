...
#
# This script is a helper to allow hibenchmarks collect tc data.
# tc output parsing has been implemented in C, inside hibenchmarks
...
if [ ! -d "${fireqos_run_dir}" ]
    ...
    if [ ! -z "${fireqos}" ]
        ...
        warning "FireQoS is not installed on this system. Use FireQoS to apply traffic QoS and expose the class names to hibenchmarks. Check https://github.com/firehol/hibenchmarks/wiki/You-should-install-QoS-on-all-your-servers"
    fi
fi

# -----------------------------------------------------------------------------

[ -z "${HIBENCHMARKS_PLUGINS_DIR}" ] && HIBENCHMARKS_PLUGINS_DIR="$(dirname "${0}")"
[ -z "${HIBENCHMARKS_CONFIG_DIR}" ] && HIBENCHMARKS_CONFIG_DIR="$(dirname "${0}")/../../../../etc/hibenchmarks"

plugins_dir="${HIBENCHMARKS_PLUGINS_DIR}"
config_dir="${HIBENCHMARKS_CONFIG_DIR}"
...
# time in seconds to exit - hibenchmarks will restart the script
qos_exit_every=3600
...
t=${1}
update_every=$((t))
[ $((update_every)) -lt 1 ] && update_every=${HIBENCHMARKS_UPDATE_EVERY}
...
show_tc() {
    ...
    # hibenchmarks can parse the output of tc

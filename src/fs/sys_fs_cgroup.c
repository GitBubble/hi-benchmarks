...
void read_cgroup_plugin_configuration() {
    ...
    snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, s);
    ...
    snprintfz(filename, FILENAME_MAX, "%s/cgroup-name.sh", hibenchmarks_configured_plugins_dir);
    cgroups_rename_script = config_get("plugin:cgroups", "script to get cgroup names", filename);

    snprintfz(filename, FILENAME_MAX, "%s/cgroup-network", hibenchmarks_configured_plugins_dir);
...
static inline char *cgroup_chart_type(char *buffer, const char *id, size_t len) {
    ...
    hibenchmarks_fix_chart_id(buffer);
    return buffer;
}
...
void *cgroups_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(cgroup_main_cleanup, ptr);

    struct rusage thread;

    // when ZERO, attempt to do it
    int vdo_cpu_hibenchmarks = config_get_boolean("plugin:cgroups", "cgroups plugin resource charts", 1);
        ...
        if(vdo_cpu_hibenchmarks) {
            ...
            if(unlikely(!stcpu_thread)) {
                ...
                stcpu_thread = rrdset_create_localhost(
                        ...
                        , "cgroups"
                        , NULL
                        , "HiBenchmarks CGroups Plugin CPU usage"
                        , "milliseconds/s"
                        , "cgroup"
                        , "stats"
                        , 132000

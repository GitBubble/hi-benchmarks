...
struct section {
    ...
    hibenchmarks_mutex_t mutex;  // this locks only the writers, to ensure atomic updates
                            // readers are protected using the rwlock in avl_tree_lock
};

static int appconfig_section_compare(void *a, void *b);

struct config hibenchmarks_config = {
        .sections = NULL,
        .mutex = HIBENCHMARKS_MUTEX_INITIALIZER,
        .index = {
...
static inline struct section *appconfig_section_create(struct config *root, const char *section) {
    ...
    hibenchmarks_mutex_init(&co->mutex);
...
void appconfig_generate(struct config *root, BUFFER *wb, int only_changed)
{
    ...
    for(i = 0; i < 3 ;i++) {
        switch(i) {
            case 0:
                buffer_strcat(wb,
                    "# hibenchmarks configuration\n"
                    "#\n"
                    "# You can download the latest version of this file, using:\n"
                    "#\n"
                    "#  wget -O /etc/hibenchmarks/hibenchmarks.conf http://localhost:19999/hibenchmarks.conf\n"
                    "# or\n"
                    "#  curl -o /etc/hibenchmarks/hibenchmarks.conf http://localhost:19999/hibenchmarks.conf\n"
                    "#\n"
                    "# You can uncomment and change any of the options below.\n"
                    "# The value shown in the commented settings, is the default value.\n"
                    "#\n"
                    "\n# global hibenchmarks configuration\n");
                break;

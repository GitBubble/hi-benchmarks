...
static inline void HIBENCHMARKS_DICTIONARY_STATS_INSERTS_PLUS1(DICTIONARY *dict) {
    if(likely(dict->stats))
        dict->stats->inserts++;
}
static inline void HIBENCHMARKS_DICTIONARY_STATS_DELETES_PLUS1(DICTIONARY *dict) {
    if(likely(dict->stats))
        dict->stats->deletes++;
}
static inline void HIBENCHMARKS_DICTIONARY_STATS_SEARCHES_PLUS1(DICTIONARY *dict) {
    if(likely(dict->stats))
        dict->stats->searches++;
}
static inline void HIBENCHMARKS_DICTIONARY_STATS_ENTRIES_PLUS1(DICTIONARY *dict) {
    if(likely(dict->stats))
        dict->stats->entries++;
}
static inline void HIBENCHMARKS_DICTIONARY_STATS_ENTRIES_MINUS1(DICTIONARY *dict) {
    if(likely(dict->stats))
        dict->stats->entries--;
}
...
static inline void dictionary_read_lock(DICTIONARY *dict) {
    if(likely(dict->rwlock)) {
        ...
        hibenchmarks_rwlock_rdlock(dict->rwlock);
    }
}
...
static inline void dictionary_write_lock(DICTIONARY *dict) {
    if(likely(dict->rwlock)) {
        ...
        hibenchmarks_rwlock_wrlock(dict->rwlock);
    }
}
...
static inline void dictionary_unlock(DICTIONARY *dict) {
    if(likely(dict->rwlock)) {
        ...
        hibenchmarks_rwlock_unlock(dict->rwlock);
    }
}
...
static inline NAME_VALUE *dictionary_name_value_index_find_nolock(DICTIONARY *dict, const char *name, uint32_t hash) {
    ...
    HIBENCHMARKS_DICTIONARY_STATS_SEARCHES_PLUS1(dict);
...
DICTIONARY *dictionary_create(uint8_t flags) {
    ...
    if(!(flags & DICTIONARY_FLAG_SINGLE_THREADED)) {
        dict->rwlock = callocz(1, sizeof(hibenchmarks_rwlock_t));
        hibenchmarks_rwlock_init(dict->rwlock);
    }
...
void dictionary_destroy(DICTIONARY *dict) {
    ...
    if(dict->rwlock) {
        hibenchmarks_rwlock_destroy(dict->rwlock);
        freez(dict->rwlock);
    }

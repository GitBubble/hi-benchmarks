...
// example configuration in hibenchmarks/conf.d/node.d/fronius.conf.md

var url = require("url");
var http = require("http");
var hibenchmarks = require("hibenchmarks");

hibenchmarks.debug("loaded " + __filename + " plugin");
...
var fronius = {
    ...
    getInverterPowerChart: function (service, suffix, inverters) {
        ...
        chart = {
            ...
            type: hibenchmarks.chartTypes.stacked,                // the type of the chart
    ...
    convertToJson: function (httpBody) {
        ...
        if (typeof httpBody !== "object") {
            try {
                ...
                hibenchmarks.error("fronius: Got a response, but it is not valid JSON. Ignoring. Error: " + error.message);
                return null;
            }
        }
    ...
    serviceExecute: function (name, uri, update_every) {
        hibenchmarks.debug(this.name + ": " + name + ": url: " + uri + ", update_every: " + update_every);

        var service = hibenchmarks.service({
            name: name,
            request: hibenchmarks.requestFromURL("http://" + uri),

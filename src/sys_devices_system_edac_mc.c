...
static void find_all_mc() {
    ...
    snprintfz(name, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/devices/system/edac/mc");
    ...
    if(do_ce == CONFIG_BOOLEAN_YES || (do_ce == CONFIG_BOOLEAN_AUTO && ce_sum > 0)) {
        ...
        if(unlikely(!ce_st)) {
            ce_st = rrdset_create_localhost(
                    ...
                    , HIBENCHMARKS_CHART_PRIO_MEM_HW + 50
                    , update_every
                    , RRDSET_TYPE_LINE
            );
        }
    ...
    if(do_ue == CONFIG_BOOLEAN_YES || (do_ue == CONFIG_BOOLEAN_AUTO && ue_sum > 0)) {
        ...
        if(unlikely(!ue_st)) {
            ue_st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "/sys/devices/system/edac/mc"
                    , HIBENCHMARKS_CHART_PRIO_MEM_HW + 60
                    , update_every
                    , RRDSET_TYPE_LINE
            );
        }

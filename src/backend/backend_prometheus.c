...
static inline time_t prometheus_server_last_access(const char *server, RRDHOST *host, time_t now) {
    static hibenchmarks_mutex_t prometheus_server_root_mutex = HIBENCHMARKS_MUTEX_INITIALIZER;

    uint32_t hash = simple_hash(server);

    hibenchmarks_mutex_lock(&prometheus_server_root_mutex);
    ...
    for(ps = prometheus_server_root; ps ;ps = ps->next) {
        if (host == ps->host && hash == ps->hash && !strcmp(server, ps->server)) {
            ...
            ps->last_access = now;
            hibenchmarks_mutex_unlock(&prometheus_server_root_mutex);
            return last;
        }
    }
...
static void rrd_stats_api_v1_charts_allmetrics_prometheus(RRDHOST *host, BUFFER *wb, const char *prefix, uint32_t options, time_t after, time_t before, int allhosts, int help, int types, int names, int timestamps) {
    ...
    if(allhosts) {
        if(timestamps)
            buffer_sprintf(wb, "hibenchmarks_info{instance=\"%s\",application=\"%s\",version=\"%s\"} 1 %llu\n", hostname, host->program_name, host->program_version, now_realtime_usec() / USEC_PER_MS);
        else
            buffer_sprintf(wb, "hibenchmarks_info{instance=\"%s\",application=\"%s\",version=\"%s\"} 1\n", hostname, host->program_name, host->program_version);

        if(host->tags && *(host->tags)) {
            if(timestamps) {
                buffer_sprintf(wb, "hibenchmarks_host_tags_info{instance=\"%s\",%s} 1 %llu\n", hostname, host->tags, now_realtime_usec() / USEC_PER_MS);

                // deprecated, exists only for compatibility with older queries
                buffer_sprintf(wb, "hibenchmarks_host_tags{instance=\"%s\",%s} 1 %llu\n", hostname, host->tags, now_realtime_usec() / USEC_PER_MS);
            }
            else {
                buffer_sprintf(wb, "hibenchmarks_host_tags_info{instance=\"%s\",%s} 1\n", hostname, host->tags);

                // deprecated, exists only for compatibility with older queries
                buffer_sprintf(wb, "hibenchmarks_host_tags{instance=\"%s\",%s} 1\n", hostname, host->tags);
            }
        ...
        if(host->tags && *(host->tags)) {
            if(timestamps) {
                buffer_sprintf(wb, "hibenchmarks_host_tags_info{%s} 1 %llu\n", host->tags, now_realtime_usec() / USEC_PER_MS);

                // deprecated, exists only for compatibility with older queries
                buffer_sprintf(wb, "hibenchmarks_host_tags{%s} 1 %llu\n", host->tags, now_realtime_usec() / USEC_PER_MS);
            }
            else {
                buffer_sprintf(wb, "hibenchmarks_host_tags_info{%s} 1\n", host->tags);

                // deprecated, exists only for compatibility with older queries
                buffer_sprintf(wb, "hibenchmarks_host_tags{%s} 1\n", host->tags);
            }
        }
    }
...
static inline time_t prometheus_preparation(RRDHOST *host, BUFFER *wb, uint32_t options, const char *server, time_t now, int help) {
    ...
    if(help) {
        ...
        buffer_sprintf(wb, "# COMMENT hibenchmarks \"%s\" to %sprometheus \"%s\", source \"%s\", last seen %lu %s"
                , host->hostname

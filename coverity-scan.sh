...
echo >&2 "Working on hibenchmarks version: ${version}"
...
[ -f hibenchmarks-coverity-analysis.tgz ] && \
	rm hibenchmarks-coverity-analysis.tgz

"${covbuild}" --dir cov-int make -j${cpus} || exit 1

echo >&2 "Compressing data..."
tar czvf hibenchmarks-coverity-analysis.tgz cov-int || exit 1

echo >&2 "Sending analysis..."
curl --progress-bar --form token="${token}" \
  --form email=costa@tsaousis.gr \
  --form file=@hibenchmarks-coverity-analysis.tgz \
  --form version="${version}" \
  --form description="hibenchmarks, real-time performance monitoring, done right." \
  https://scan.coverity.com/builds?project=firehol%2Fhibenchmarks

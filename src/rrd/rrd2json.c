...
void rrd_stats_api_v1_charts_allmetrics_shell(RRDHOST *host, BUFFER *wb) {
        ...
        if(rrdset_is_available_for_viewers(st)) {
            ...
            rrddim_foreach_read(rd, st) {
                if(rd->collections_counter) {
                    ...
                    if(isnan(n) || isinf(n))
                        buffer_sprintf(wb, "HIBENCHMARKS_%s_%s=\"\"      # %s\n", chart, dimension, st->units);
                    else {
                        if(rd->multiplier < 0 || rd->divisor < 0) n = -n;
                        n = calculated_number_round(n);
                        if(!rrddim_flag_check(rd, RRDDIM_FLAG_HIDDEN)) total += n;
                        buffer_sprintf(wb, "HIBENCHMARKS_%s_%s=\"" CALCULATED_NUMBER_FORMAT_ZERO "\"      # %s\n", chart, dimension, n, st->units);
                    }
                }
            }

            total = calculated_number_round(total);
            buffer_sprintf(wb, "HIBENCHMARKS_%s_VISIBLETOTAL=\"" CALCULATED_NUMBER_FORMAT_ZERO "\"      # %s\n", chart, total, st->units);
            rrdset_unlock(st);
        }
    }

    buffer_strcat(wb, "\n# HIBENCHMARKS ALARMS RUNNING\n");
    ...
    for(rc = host->alarms; rc ;rc = rc->next) {
        ...
        if(isnan(n) || isinf(n))
            buffer_sprintf(wb, "HIBENCHMARKS_ALARM_%s_%s_VALUE=\"\"      # %s\n", chart, alarm, rc->units);
        else {
            n = calculated_number_round(n);
            buffer_sprintf(wb, "HIBENCHMARKS_ALARM_%s_%s_VALUE=\"" CALCULATED_NUMBER_FORMAT_ZERO "\"      # %s\n", chart, alarm, n, rc->units);
        }

        buffer_sprintf(wb, "HIBENCHMARKS_ALARM_%s_%s_STATUS=\"%s\"\n", chart, alarm, rrdcalc_status2string(rc->status));
    }

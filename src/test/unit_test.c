...
static int check_number_printing(void) {
    ...
    char hibenchmarks[50], system[50];
    int i, failed = 0;
    for(i = 0; values[i].correct ; i++) {
        print_calculated_number(hibenchmarks, values[i].n);
        snprintfz(system, 49, "%0.12" LONG_DOUBLE_MODIFIER, (LONG_DOUBLE)values[i].n);

        int ok = 1;
        if(strcmp(hibenchmarks, values[i].correct) != 0) {
            ok = 0;
            failed++;
        }

        fprintf(stderr, "'%s' (system) printed as '%s' (hibenchmarks): %s\n", system, hibenchmarks, ok?"OK":"FAILED");
    }
...
void benchmark_storage_number(int loop, int multiplier) {
    ...
    fprintf(stderr, "HIBENCHMARKS FLOATING POINT SIZE: %zu bytes\n", sizeof(storage_number));
    ...
    if(mine > their) {
        fprintf(stderr, "\nHIBENCHMARKS NEEDS %0.2" LONG_DOUBLE_MODIFIER " TIMES MORE MEMORY. Sorry!\n", (LONG_DOUBLE)(mine / their));
    }
    else {
        fprintf(stderr, "\nHIBENCHMARKS INTERNAL FLOATING POINT ARITHMETICS NEEDS %0.2" LONG_DOUBLE_MODIFIER " TIMES LESS MEMORY.\n", (LONG_DOUBLE)(their / mine));
    }

    fprintf(stderr, "\nHIBENCHMARKS FLOATING POINT\n");
    ...
    if(mine > total) {
        fprintf(stderr, "HIBENCHMARKS CODE IS SLOWER %0.2" LONG_DOUBLE_MODIFIER " %%\n", (LONG_DOUBLE)(mine * 100.0 / their - 100.0));
    }
    else {
        fprintf(stderr, "HIBENCHMARKS CODE IS  F A S T E R  %0.2" LONG_DOUBLE_MODIFIER " %%\n", (LONG_DOUBLE)(their * 100.0 / mine - 100.0));
    }
    ...
    if(mine > their) {
        fprintf(stderr, "WITH PACKING UNPACKING HIBENCHMARKS CODE IS SLOWER %0.2" LONG_DOUBLE_MODIFIER " %%\n", (LONG_DOUBLE)(mine * 100.0 / their - 100.0));
    }
    else {
        fprintf(stderr, "EVEN WITH PACKING AND UNPACKING, HIBENCHMARKS CODE IS  F A S T E R  %0.2" LONG_DOUBLE_MODIFIER " %%\n", (LONG_DOUBLE)(their * 100.0 / mine - 100.0));
    }
...
int run_test(struct test *test)
{
    ...
    // create the chart
    RRDSET *st = rrdset_create_localhost("hibenchmarks", name, name, "hibenchmarks", NULL, "Unit Testing", "a value", "unittest", NULL, 1
...
int unit_test(long delay, long shift)
{
    ...
    RRDSET *st = rrdset_create_localhost("hibenchmarks", name, name, "hibenchmarks", NULL, "Unit Testing", "a value", "unittest", NULL, 1, 1
                                         , RRDSET_TYPE_LINE);
    rrdset_flag_set(st, RRDSET_FLAG_DEBUG);

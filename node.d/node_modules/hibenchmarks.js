...
var example_chart = {
    ...
    dimensions: {
        ...
        'dim2': {
            ...
            name: 'name',           // the name of the dimension
            algorithm: 'absolute',  // the id of the hibenchmarks algorithm
            multiplier: 1,          // the multiplier
            divisor: 1,             // the divisor
            hidden: false,          // is hidden (boolean)
        }
...
var hibenchmarks = {
    ...
    processors: {
        http: {
            ...
            process: function(service, callback) {
                ...
                if(__DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': making ' + this.name + ' request: ' + hibenchmarks.stringify(service.request));

                var req = http.request(service.request, function(response) {
                    if(__DEBUG === true) hibenchmarks.debug(service.module.name + ': ' + service.name + ': got server response...');
                    ...
                    response.on('end', function() {
                        if(end === false) {
                            if(__DEBUG === true) hibenchmarks.debug(service.module.name + ': ' + service.name + ': read completed.');
                            end = true;
                ...
                req.on('error', function(e) {
                    if(__DEBUG === true) hibenchmarks.debug('Failed to make request: ' + hibenchmarks.stringify(service.request) + ', message: ' + e.message);
                ...
                if(typeof service.postData !== 'undefined' && service.request.method === 'POST') {
                    if(__DEBUG === true) hibenchmarks.debug(service.module.name + ': ' + service.name + ': posting data: ' + service.postData);
    ...
    debug: function(msg) {
        if(this.options.DEBUG === true) {
            console.error(this.logdate() + ': ' + hibenchmarks.options.filename + ': DEBUG: ' + ((typeof(msg) === 'object')?hibenchmarks.stringify(msg):msg).toString());
        }
    },

    // log an error
    error: function(msg) {
        console.error(this.logdate() + ': ' + hibenchmarks.options.filename + ': ERROR: ' + ((typeof(msg) === 'object')?hibenchmarks.stringify(msg):msg).toString());
    },

    // send data to hibenchmarks
    send: function(msg) {
    ...
    service: function(service) {
        ...
        service._queue = '';            // data to be sent to hibenchmarks

        service.error_reported = false; // error log flood control

        service.added = false;          // added to hibenchmarks.services
        ...
        if(typeof service.processor === 'undefined')
            service.processor = hibenchmarks.processors.http;

        if(typeof service.update_every === 'undefined')
            service.update_every = service.module.update_every;

        if(typeof service.update_every === 'undefined')
            service.update_every = hibenchmarks.options.update_every;

        if(service.update_every < hibenchmarks.options.update_every)
            service.update_every = hibenchmarks.options.update_every;
        ...
        service.commit = function() {
            if(this.added !== true) {
                ...
                hibenchmarks.services.push(this);
                if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug(this.module.name + ': ' + this.name + ': service committed.');
            }
        };
        ...
        service.execute = function(responseProcessor) {
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(this.module.name + ': ' + this.name + ': making ' + this.processor.name + ' request: ' + hibenchmarks.stringify(this));
            ...
            this.processor.process(this, function(response) {
                ...
                if(__DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': processing ' + service.processor.name + ' response (received in ' + (service.ended - service.started).toString() + ' ms)');

                try {
                    responseProcessor(service, response);
                }
                catch(e) {
                    hibenchmarks.error(e);
                ...
                if(service.module.active < 0) {
                    ...
                    if(__DEBUG === true)
                        hibenchmarks.debug(service.module.name + ': active module counter below zero.');
                }
                ...
                if(service.module.active === 0) {
                    ...
                    if(service.module.configure_callback !== null) {
                        if(__DEBUG === true)
                            hibenchmarks.debug(service.module.name + ': configuration finish callback called from processResponse().');
        ...
        service.update = function() {
            if(hibenchmarks.options.DEBUG === true)
                hibenchmarks.debug(this.module.name + ': ' + this.name + ': starting data collection...');

            this.module.update(this, function() {
                if(hibenchmarks.options.DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': data collection ended in ' + service.duration.toString() + ' ms.');
            });
        };

        service.error = function(message) {
            if(this.error_reported === false) {
                hibenchmarks.error(this.module.name + ': ' + this.name + ': ' + message);
                this.error_reported = true;
            }
            else if(hibenchmarks.options.DEBUG === true)
                hibenchmarks.debug(this.module.name + ': ' + this.name + ': ' + message);
        };
        ...
        service._send_chart_to_hibenchmarks = function(chart) {
            // internal function to send a chart to hibenchmarks
        ...
        service.begin = function(chart) {
            ...
            if(typeof(chart.id) === 'undefined' || hibenchmarks.charts[chart.id] !== chart) {
                this.error('Called begin() for chart ' + chart.id + ' that is not mine. Where did you find it? Ignoring it.');
                return false;
            }

            if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug('setting current chart to ' + chart.id);
            ...
            if(this._current_chart._dimensions_count !== 0) {
                if(this._current_chart._created === false || this._current_chart._updated === true)
                    this._send_chart_to_hibenchmarks(this._current_chart);
        ...
        service.end = function() {
            ...
            if(this._current_chart._dimensions_count !== 0) {
                this.queue('END');
                hibenchmarks.send(this._queue);
            }

            this._queue = '';
            this._current_chart._began = false;
            if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug('sent chart ' + this._current_chart.id);
        ...
        // create a hibenchmarks chart
        service.chart = function(id, chart) {
            var __DEBUG = hibenchmarks.options.DEBUG;

            if(typeof(hibenchmarks.charts[id]) === 'undefined') {
                hibenchmarks.charts[id] = {
                    _created: false,
                    _updated: true,
                    _began: false,
                    _counter: 0,
                    ...
                    family: '',
                    context: '',
                    type: hibenchmarks.chartTypes.line,
                    priority: 50000,
            ...
            var c = hibenchmarks.charts[id];

            if(typeof(chart.name) !== 'undefined' && chart.name !== c.name) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its name');
                c.name = chart.name;
                c._updated = true;
            }

            if(typeof(chart.title) !== 'undefined' && chart.title !== c.title) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its title');
                c.title = chart.title;
                c._updated = true;
            }

            if(typeof(chart.units) !== 'undefined' && chart.units !== c.units) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its units');
                c.units = chart.units;
                c._updated = true;
            }

            if(typeof(chart.family) !== 'undefined' && chart.family !== c.family) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its family');
                c.family = chart.family;
                c._updated = true;
            }

            if(typeof(chart.context) !== 'undefined' && chart.context !== c.context) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its context');
                c.context = chart.context;
                c._updated = true;
            }

            if(typeof(chart.type) !== 'undefined' && chart.type !== c.type) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its type');
                c.type = chart.type;
                c._updated = true;
            }

            if(typeof(chart.priority) !== 'undefined' && chart.priority !== c.priority) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its priority');
                c.priority = chart.priority;
                c._updated = true;
            }

            if(typeof(chart.update_every) !== 'undefined' && chart.update_every !== c.update_every) {
                if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' updated its update_every from ' + c.update_every + ' to ' + chart.update_every);
            ...
            if(typeof(chart.dimensions) !== 'undefined') {
                ...
                while(len--) {
                    ...
                    if(typeof(c.dimensions[x]) === 'undefined') {
                        ...
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ' created dimension ' + x);
                        c._updated = true;
                    }
                    ...
                    if(typeof(dim.name) !== 'undefined' && d.name !== dim.name) {
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ', dimension ' + x + ' updated its name');
                        d.name = dim.name;
                        d._updated = true;
                    }

                    if(typeof(dim.algorithm) !== 'undefined' && d.algorithm !== dim.algorithm) {
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ', dimension ' + x + ' updated its algorithm from ' + d.algorithm + ' to ' + dim.algorithm);
                        d.algorithm = dim.algorithm;
                        d._updated = true;
                    }

                    if(typeof(dim.multiplier) !== 'undefined' && d.multiplier !== dim.multiplier) {
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ', dimension ' + x + ' updated its multiplier');
                        d.multiplier = dim.multiplier;
                        d._updated = true;
                    }

                    if(typeof(dim.divisor) !== 'undefined' && d.divisor !== dim.divisor) {
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ', dimension ' + x + ' updated its divisor');
                        d.divisor = dim.divisor;
                        d._updated = true;
                    }

                    if(typeof(dim.hidden) !== 'undefined' && d.hidden !== dim.hidden) {
                        if(__DEBUG === true) hibenchmarks.debug('chart ' + id + ', dimension ' + x + ' updated its hidden status');
                        d.hidden = dim.hidden;
                        d._updated = true;
                    }
            ...
            //if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug(hibenchmarks.charts);
            return hibenchmarks.charts[id];
        };
    ...
    runAllServices: function() {
        if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug('runAllServices()');

        var now = Date.now();
        var len = hibenchmarks.services.length;
        while(len--) {
            var service = hibenchmarks.services[len];
        ...
        setTimeout(hibenchmarks.runAllServices, hibenchmarks.options.update_every * 100);
    },

    start: function() {
        if(hibenchmarks.options.DEBUG === true) this.debug('started, services: ' + hibenchmarks.stringify(this.services));
    ...
    requestFromParams: function(protocol, hostname, port, path, method) {
        return {
            ...
            agent: new http.Agent({
                keepAlive: true,
                keepAliveMsecs: hibenchmarks.options.update_every * 1000,
    ...
    requestFromURL: function(a_url) {
        ...
        return hibenchmarks.requestFromParams(u.protocol, u.hostname, u.port, u.path, 'GET');
    },

    configure: function(module, config, callback) {
        if(hibenchmarks.options.DEBUG === true) this.debug(module.name + ': configuring (update_every: ' + this.options.update_every + ')...');
        ...
        if(hibenchmarks.options.DEBUG === true) this.debug(module.name + ': configured, reporting ' + added + ' eligible services.');

        if(module.configure_callback !== null && added === 0) {
            if(hibenchmarks.options.DEBUG === true) this.debug(module.name + ': configuration finish callback called from configure().');
...
if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug('loaded hibenchmarks from:', __filename);
module.exports = hibenchmarks;

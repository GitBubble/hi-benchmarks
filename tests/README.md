...
The dependencies are installed in `hibenchmarks/package.json`. If you install a new NPM module, it gets added here. Future developers just need to execute `npm install` and every dep gets added automatically.
...
3. - Name: Karma Headless Chromium
   - Configuration file: /path/to/your/hibenchmarks/tests/web/karma.conf.js
   - Browsers to start: ChromiumHeadless
   - Node interpreter: /usr/bin/nodejs (MUST be absolute, NVM works too)
   - Karma package: /path/to/your/hibenchmarks/node_modules/karma
...
A nice HTML is produced from Karma which shows which code paths were executed. It is located somewhere in `/path/to/your/hibenchmarks/coverage/`

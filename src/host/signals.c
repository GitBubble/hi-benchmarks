...
typedef enum signal_action {
    HIBENCHMARKS_SIGNAL_END_OF_LIST,
    HIBENCHMARKS_SIGNAL_IGNORE,
    HIBENCHMARKS_SIGNAL_EXIT_CLEANLY,
    HIBENCHMARKS_SIGNAL_SAVE_DATABASE,
    HIBENCHMARKS_SIGNAL_LOG_ROTATE,
    HIBENCHMARKS_SIGNAL_RELOAD_HEALTH,
    HIBENCHMARKS_SIGNAL_FATAL,
} SIGNAL_ACTION;
...
static struct {
    ...
    SIGNAL_ACTION action;   // the action to take
        ...
        { SIGPIPE, "SIGPIPE", 0, HIBENCHMARKS_SIGNAL_IGNORE        },
        { SIGINT , "SIGINT",  0, HIBENCHMARKS_SIGNAL_EXIT_CLEANLY  },
        { SIGQUIT, "SIGQUIT", 0, HIBENCHMARKS_SIGNAL_EXIT_CLEANLY  },
        { SIGTERM, "SIGTERM", 0, HIBENCHMARKS_SIGNAL_EXIT_CLEANLY  },
        { SIGHUP,  "SIGHUP",  0, HIBENCHMARKS_SIGNAL_LOG_ROTATE    },
        { SIGUSR1, "SIGUSR1", 0, HIBENCHMARKS_SIGNAL_SAVE_DATABASE },
        { SIGUSR2, "SIGUSR2", 0, HIBENCHMARKS_SIGNAL_RELOAD_HEALTH },
        { SIGBUS,  "SIGBUS",  0, HIBENCHMARKS_SIGNAL_FATAL         },

        // terminator
        { 0,       "NONE",    0, HIBENCHMARKS_SIGNAL_END_OF_LIST   }
};

static void signal_handler(int signo) {
    // find the entry in the list
    int i;
    for(i = 0; signals_waiting[i].action != HIBENCHMARKS_SIGNAL_END_OF_LIST ; i++) {
        if(unlikely(signals_waiting[i].signo == signo)) {
            signals_waiting[i].count++;

            if(signals_waiting[i].action == HIBENCHMARKS_SIGNAL_FATAL) {
                char buffer[200 + 1];
...
void signals_init(void) {
    ...
    int i;
    for (i = 0; signals_waiting[i].action != HIBENCHMARKS_SIGNAL_END_OF_LIST; i++) {
        if(signals_waiting[i].action == HIBENCHMARKS_SIGNAL_IGNORE)
                ...
                for (i = 0; signals_waiting[i].action != HIBENCHMARKS_SIGNAL_END_OF_LIST; i++) {
                    if (signals_waiting[i].count) {
                        ...
                        switch (signals_waiting[i].action) {
                            case HIBENCHMARKS_SIGNAL_RELOAD_HEALTH:
                            ...
                            case HIBENCHMARKS_SIGNAL_SAVE_DATABASE:
                                error_log_limit_unlimited();
                            ...
                            case HIBENCHMARKS_SIGNAL_LOG_ROTATE:
                                error_log_limit_unlimited();
                            ...
                            case HIBENCHMARKS_SIGNAL_EXIT_CLEANLY:
                                error_log_limit_unlimited();
                                info("SIGNAL: Received %s. Cleaning up to exit...", name);
                                hibenchmarks_cleanup_and_exit(0);
                                exit(0);

                            case HIBENCHMARKS_SIGNAL_FATAL:
                                fatal("SIGNAL: Received %s. hibenchmarks now exits.", name);

...
void *tc_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(tc_main_cleanup, ptr);
...
#ifdef DETACH_PLUGINS_FROM_HIBENCHMARKS
    uint32_t MYPID_HASH = simple_hash("MYPID");
#endif
    uint32_t first_hash;

    snprintfz(buffer, TC_LINE_MAX, "%s/tc-qos-helper.sh", hibenchmarks_configured_plugins_dir);

...
hibenchmarks_rwlock_t rrd_rwlock = HIBENCHMARKS_RWLOCK_INITIALIZER;
...
RRDHOST *rrdhost_create(const char *hostname,
    ...
    hibenchmarks_mutex_init(&host->rrdpush_sender_buffer_mutex);
    hibenchmarks_rwlock_init(&host->rrdhost_rwlock);
    ...
    hibenchmarks_rwlock_init(&host->health_log.alarm_log_rwlock);

    char filename[FILENAME_MAX + 1];

    if(is_localhost) {

        host->cache_dir  = strdupz(hibenchmarks_configured_cache_dir);
        host->varlib_dir = strdupz(hibenchmarks_configured_varlib_dir);

    }
    else {
        // this is not localhost - append our GUID to localhost path

        snprintfz(filename, FILENAME_MAX, "%s/%s", hibenchmarks_configured_cache_dir, host->machine_guid);
        ...
        snprintfz(filename, FILENAME_MAX, "%s/%s", hibenchmarks_configured_varlib_dir, host->machine_guid);
    ...
    snprintfz(filename, FILENAME_MAX, "%s/alarm-notify.sh", hibenchmarks_configured_plugins_dir);
...
RRDHOST *rrdhost_find_or_create(
    ...
    if(!host) {
        ...
        if(host->rrd_update_every != update_every)
            error("Host '%s' has an update frequency of %d seconds, but the wanted one is %d seconds. Restart hibenchmarks here to apply the new settings.", host->hostname, host->rrd_update_every, update_every);

        if(host->rrd_history_entries < history)
            error("Host '%s' has history of %ld entries, but the wanted one is %ld entries. Restart hibenchmarks here to apply the new settings.", host->hostname, host->rrd_history_entries, history);

        if(host->rrd_memory_mode != mode)
            error("Host '%s' has memory mode '%s', but the wanted one is '%s'. Restart hibenchmarks here to apply the new settings.", host->hostname, rrd_memory_mode_name(host->rrd_memory_mode), rrd_memory_mode_name(mode));
...
void rrd_init(char *hostname) {
    ...
    localhost = rrdhost_create(
            ...
            , os_type
            , hibenchmarks_configured_timezone
...
// there are only used when HIBENCHMARKS_INTERNAL_CHECKS is set

void __rrdhost_check_rdlock(RRDHOST *host, const char *file, const char *function, const unsigned long line) {
    debug(D_RRDHOST, "Checking read lock on host '%s'", host->hostname);

    int ret = hibenchmarks_rwlock_trywrlock(&host->rrdhost_rwlock);
    if(ret == 0)
...
void __rrdhost_check_wrlock(RRDHOST *host, const char *file, const char *function, const unsigned long line) {
    ...
    int ret = hibenchmarks_rwlock_tryrdlock(&host->rrdhost_rwlock);
    if(ret == 0)
...
void __rrd_check_rdlock(const char *file, const char *function, const unsigned long line) {
    ...
    int ret = hibenchmarks_rwlock_trywrlock(&rrd_rwlock);
    if(ret == 0)
...
void __rrd_check_wrlock(const char *file, const char *function, const unsigned long line) {
    ...
    int ret = hibenchmarks_rwlock_tryrdlock(&rrd_rwlock);
    if(ret == 0)
...
void rrdhost_free(RRDHOST *host) {
    ...
    rrdhost_unlock(host);
    hibenchmarks_rwlock_destroy(&host->health_log.alarm_log_rwlock);
    hibenchmarks_rwlock_destroy(&host->rrdhost_rwlock);
    freez(host);

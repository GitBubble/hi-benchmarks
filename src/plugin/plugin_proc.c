...
void *proc_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(proc_main_cleanup, ptr);

    int vdo_cpu_hibenchmarks = config_get_boolean("plugin:proc", "hibenchmarks server resources", 1);
    ...
    while(!hibenchmarks_exit) {
        ...
        if(vdo_cpu_hibenchmarks) {
            ...
            if(unlikely(!st)) {
                st = rrdset_find_bytype_localhost("hibenchmarks", "plugin_proc_modules");
...
int get_numa_node_count(void)
{
    ...
    snprintfz(name, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/devices/system/node");

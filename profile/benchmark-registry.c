...
/*
 * compile with
 *  gcc -g -ggdb -Wall -Wextra -I ../src/ -I ../ -o benchmark-registry benchmark-registry.c ../src/dictionary.o ../src/log.o ../src/avl.o ../src/common.o ../src/appconfig.o ../src/web_buffer.o ../src/storage_number.o ../src/rrd.o ../src/health.o -pthread -luuid -lm -DHAVE_CONFIG_H -DVARLIB_DIR="\"/tmp\""
 */
...
int test1(int argc, char **argv) {
	...
	for(m = 0; m < machines2 ;m++) {
		...
		snprintfz(buf, FILENAME_MAX, "http://%u.hibenchmarks.rocks/", m+1);
	...
	for(m = 0; m < 10; m++) {
		...
		for (u = 0; u < users * 2; u++) {
			...
			if (random() % 10000 == 1234) {
				snprintfz(buf, FILENAME_MAX, "http://random.%ld.hibenchmarks.rocks/", random());
				url = buf;
			}

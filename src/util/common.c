...
char *hibenchmarks_configured_hostname    = NULL;
char *hibenchmarks_configured_config_dir  = NULL;
char *hibenchmarks_configured_log_dir     = NULL;
char *hibenchmarks_configured_plugins_dir = NULL;
char *hibenchmarks_configured_web_dir     = NULL;
char *hibenchmarks_configured_cache_dir   = NULL;
char *hibenchmarks_configured_varlib_dir  = NULL;
char *hibenchmarks_configured_home_dir    = NULL;
char *hibenchmarks_configured_host_prefix = NULL;
char *hibenchmarks_configured_timezone    = NULL;

struct rlimit rlimit_nofile = { .rlim_cur = 1024, .rlim_max = 1024 };
int enable_ksm = 1;

volatile sig_atomic_t hibenchmarks_exit = 0;
const char *os_type = HIBENCHMARKS_OS_TYPE;
const char *program_version = VERSION;

// ----------------------------------------------------------------------------
// memory allocation functions that handle failures

// although hibenchmarks does not use memory allocations too often (hibenchmarks tries to
...
#if defined(HAVE_C___ATOMIC) && !defined(HIBENCHMARKS_NO_ATOMIC_INSTRUCTIONS)
...
#ifndef HIBENCHMARKS_WITH_USLEEP
...
unsigned char hibenchmarks_map_chart_names[256] = {
        [0] = '\0', //
        [1] = '_', //
        [2] = '_', //
        [3] = '_', //
...
// is good for a hibenchmarks chart/dimension ID/NAME
void hibenchmarks_fix_chart_name(char *s) {
    while ((*s = hibenchmarks_map_chart_names[(unsigned char) *s])) s++;
}

unsigned char hibenchmarks_map_chart_ids[256] = {
        [0] = '\0', //
        [1] = '_', //
        [2] = '_', //
        [3] = '_', //
...
// make sure the supplied string
// is good for a hibenchmarks chart/dimension ID/NAME
void hibenchmarks_fix_chart_id(char *s) {
    while ((*s = hibenchmarks_map_chart_ids[(unsigned char) *s])) s++;
}
...
pid_t get_system_pid_max(void) {
    ...
    char filename[FILENAME_MAX + 1];
    snprintfz(filename, FILENAME_MAX, "%s/proc/sys/kernel/pid_max", hibenchmarks_configured_host_prefix);
...
int verify_hibenchmarks_host_prefix() {
    if(!hibenchmarks_configured_host_prefix)
        hibenchmarks_configured_host_prefix = "";

    if(!*hibenchmarks_configured_host_prefix)
        return 0;

    char buffer[FILENAME_MAX + 1];
    char *path = hibenchmarks_configured_host_prefix;
    ...
    path = buffer;
    snprintfz(path, FILENAME_MAX, "%s/proc", hibenchmarks_configured_host_prefix);
    if(is_virtual_filesystem(path, &reason) == -1)
        goto failed;

    snprintfz(path, FILENAME_MAX, "%s/sys", hibenchmarks_configured_host_prefix);
    if(is_virtual_filesystem(path, &reason) == -1)
        goto failed;

    if(hibenchmarks_configured_host_prefix && *hibenchmarks_configured_host_prefix)
        info("Using host prefix directory '%s'", hibenchmarks_configured_host_prefix);

    return 0;

failed:
    error("Ignoring host prefix '%s': path '%s' %s", hibenchmarks_configured_host_prefix, path, reason);
    hibenchmarks_configured_host_prefix = "";
    return -1;
}

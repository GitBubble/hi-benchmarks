...
static int read_tcp_mem(void) {
    ...
    if(unlikely(!filename)) {
        ...
        snprintfz(buffer, FILENAME_MAX, "%s/proc/sys/net/ipv4/tcp_mem", hibenchmarks_configured_host_prefix);
...
static kernel_uint_t read_tcp_max_orphans(void) {
    ...
    if(unlikely(!filename)) {
        char buffer[FILENAME_MAX + 1];
        snprintfz(buffer, FILENAME_MAX, "%s/proc/sys/net/ipv4/tcp_max_orphans", hibenchmarks_configured_host_prefix);
        filename = strdupz(buffer);
    }
...
int do_proc_net_sockstat(int update_every, usec_t dt) {
    ...
    if(unlikely(!ff)) {
        ...
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/net/sockstat");
    ...
    if(do_sockets == CONFIG_BOOLEAN_YES || (do_sockets == CONFIG_BOOLEAN_AUTO && sockstat_root.sockets_used)) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_SOCKETS
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_tcp_sockets == CONFIG_BOOLEAN_YES || (do_tcp_sockets == CONFIG_BOOLEAN_AUTO && (sockstat_root.tcp_inuse || sockstat_root.tcp_orphan || sockstat_root.tcp_tw || sockstat_root.tcp_alloc))) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_TCP + 1
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_tcp_mem == CONFIG_BOOLEAN_YES || (do_tcp_mem == CONFIG_BOOLEAN_AUTO && sockstat_root.tcp_mem)) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_TCP_MEM
                    , update_every
                    , RRDSET_TYPE_AREA
            );
    ...
    if(do_udp_mem == CONFIG_BOOLEAN_YES || (do_udp_mem == CONFIG_BOOLEAN_AUTO && sockstat_root.udp_mem)) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_UDP_MEM
                    , update_every
                    , RRDSET_TYPE_AREA
            );
    ...
    if(do_raw_sockets == CONFIG_BOOLEAN_YES || (do_raw_sockets == CONFIG_BOOLEAN_AUTO && sockstat_root.raw_inuse)) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_RAW
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_frag_mem == CONFIG_BOOLEAN_YES || (do_frag_mem == CONFIG_BOOLEAN_AUTO && sockstat_root.frag_memory)) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/sockstat"
                    , HIBENCHMARKS_CHART_PRIO_IPV4_FRAGMENTS_MEM
                    , update_every
                    , RRDSET_TYPE_AREA
            );

...
#ifndef HIBENCHMARKS_RRD_H
#define HIBENCHMARKS_RRD_H 1
...
#define RRDSET_MAGIC        "HIBENCHMARKS RRD SET FILE V019"
#define RRDDIMENSION_MAGIC  "HIBENCHMARKS RRD DIMENSION FILE V019"
...
struct rrddim {
    ...
    int exposed:1;                                  // 1 when set what have sent this dimension to the central hibenchmarks
...
typedef enum rrdset_flags {
    ...
    RRDSET_FLAG_EXPOSED_UPSTREAM = 1 << 6, // if set, we have sent this chart to hibenchmarks master (streaming)
...
struct rrdset {
    ...
    int gap_when_lost_iterations_above;             // after how many lost iterations a gap should be stored
                                                    // hibenchmarks will interpolate values for gaps lower than this
    ...
    hibenchmarks_rwlock_t rrdset_rwlock;                 // protects dimensions linked list
...
#define rrdset_rdlock(st) hibenchmarks_rwlock_rdlock(&((st)->rrdset_rwlock))
#define rrdset_wrlock(st) hibenchmarks_rwlock_wrlock(&((st)->rrdset_rwlock))
#define rrdset_unlock(st) hibenchmarks_rwlock_unlock(&((st)->rrdset_rwlock))
...
struct rrdhost {
    ...
    int rrdpush_send_enabled:1;                     // 1 when this host sends metrics to another hibenchmarks
    char *rrdpush_send_destination;                 // where to send metrics to
    char *rrdpush_send_api_key;                     // the api key at the receiving hibenchmarks

    // the following are state information for the threading
    // streaming metrics from this hibenchmarks to an upstream hibenchmarks
    volatile int rrdpush_sender_spawn:1;            // 1 when the sender thread has been spawn
    hibenchmarks_thread_t rrdpush_sender_thread;         // the sender thread
    ...
    hibenchmarks_mutex_t rrdpush_sender_buffer_mutex;    // exclusive access to rrdpush_sender_buffer
    ...
    hibenchmarks_rwlock_t rrdhost_rwlock;                // lock for this RRDHOST (protects rrdset_root linked list)
...
#define rrdhost_rdlock(host) hibenchmarks_rwlock_rdlock(&((host)->rrdhost_rwlock))
#define rrdhost_wrlock(host) hibenchmarks_rwlock_wrlock(&((host)->rrdhost_rwlock))
#define rrdhost_unlock(host) hibenchmarks_rwlock_unlock(&((host)->rrdhost_rwlock))
...
extern hibenchmarks_rwlock_t rrd_rwlock;

#define rrd_rdlock() hibenchmarks_rwlock_rdlock(&rrd_rwlock)
#define rrd_wrlock() hibenchmarks_rwlock_wrlock(&rrd_rwlock)
#define rrd_unlock() hibenchmarks_rwlock_unlock(&rrd_rwlock)
...
#if defined(HIBENCHMARKS_INTERNAL_CHECKS) && defined(HIBENCHMARKS_VERIFY_LOCKS)
...
#ifdef HIBENCHMARKS_RRD_INTERNALS
...
#endif /* HIBENCHMARKS_RRD_INTERNALS */


#endif /* HIBENCHMARKS_RRD_H */

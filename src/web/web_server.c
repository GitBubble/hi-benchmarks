...
// 3. static-threaded, based on poll() using a fixed number of threads (configured at hibenchmarks.conf)
...
SIMPLE_PATTERN *web_allow_hibenchmarksconf_from = NULL;
...
static struct web_client *web_client_alloc(void) {
    ...
    w->response.data = buffer_create(HIBENCHMARKS_WEB_RESPONSE_INITIAL_SIZE);
    w->response.header = buffer_create(HIBENCHMARKS_WEB_RESPONSE_HEADER_SIZE);
    w->response.header_output = buffer_create(HIBENCHMARKS_WEB_RESPONSE_HEADER_SIZE);
    return w;
}
...
static void *multi_threaded_web_client_worker_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(multi_threaded_web_client_worker_main_cleanup, ptr);
            ...
            while(!hibenchmarks_exit) {
                ...
                if(unlikely(hibenchmarks_exit)) break;
...
// 3. spawns a new hibenchmarks_thread to serve the client (this is optimal for keep-alive clients)
// 4. cleans up old web_clients that their hibenchmarks_threads have been exited
...
static void web_client_multi_threaded_web_server_stop_all_threads(void) {
    ...
    for(w = web_clients_cache.used; w ; w = w->next) {
        if(w->running) {
            ...
            hibenchmarks_thread_cancel(w->thread);
        }
    }
...
void *socket_listen_main_multi_threaded(void *ptr) {
    hibenchmarks_thread_cleanup_push(socket_listen_main_multi_threaded_cleanup, ptr);
    ...
    while(!hibenchmarks_exit) {
        ...
        for(i = 0 ; i < api_sockets.opened ; i++) {
            ...
            if(revents & POLLIN || revents & POLLPRI) {
                ...
                snprintfz(tag, HIBENCHMARKS_THREAD_TAG_MAX, "WEB_CLIENT[%llu,[%s]:%s]", w->id, w->client_ip, w->client_port);

                w->running = 1;
                if(hibenchmarks_thread_create(&w->thread, tag, HIBENCHMARKS_THREAD_OPTION_DONT_LOG, multi_threaded_web_client_worker_main, w) != 0) {
                    w->running = 0;
                    web_client_release(w);
                }
            }
...
void *socket_listen_main_single_threaded(void *ptr) {
    hibenchmarks_thread_cleanup_push(socket_listen_main_single_threaded_cleanup, ptr);
...
static void web_server_tmr_callback(void *timer_data) {
    ...
    if(unlikely(!st)) {
        ...
        snprintfz(title, 100, "HiBenchmarks web server thread No %d CPU usage", worker_private->id + 1);
...
void *socket_listen_main_static_threaded_worker(void *ptr) {
    ...
    hibenchmarks_thread_cleanup_push(socket_listen_main_static_threaded_worker_cleanup, ptr);
...
static void socket_listen_main_static_threaded_cleanup(void *ptr) {
    struct hibenchmarks_static_thread *static_thread = (struct hibenchmarks_static_thread *)ptr;
    static_thread->enabled = HIBENCHMARKS_MAIN_THREAD_EXITING;
    ...
    for(i = 1; i < static_threaded_workers_count; i++) {
        if(static_workers_private_data[i].running) {
            ...
            hibenchmarks_thread_cancel(static_workers_private_data[i].thread);
        }
        else
...
void *socket_listen_main_static_threaded(void *ptr) {
    hibenchmarks_thread_cleanup_push(socket_listen_main_static_threaded_cleanup, ptr);
            ...
            for(i = 1; i < static_threaded_workers_count; i++) {
                ...
                hibenchmarks_thread_create(&static_workers_private_data[i].thread, tag, HIBENCHMARKS_THREAD_OPTION_DEFAULT, socket_listen_main_static_threaded_worker,

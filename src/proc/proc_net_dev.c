...
static hibenchmarks_mutex_t netdev_rename_mutex = HIBENCHMARKS_MUTEX_INITIALIZER;
...
void netdev_rename_device_add(const char *host_device, const char *container_device, const char *container_name) {
    hibenchmarks_mutex_lock(&netdev_rename_mutex);
    ...
    hibenchmarks_mutex_unlock(&netdev_rename_mutex);
}
...
int do_proc_net_dev(int update_every, usec_t dt) {
    ...
    if(unlikely(enable_new_interfaces == -1)) {
        ...
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/devices/virtual/net/%s");
    ...
    if(unlikely(!ff)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, (*hibenchmarks_configured_host_prefix)?"/proc/1/net/dev":"/proc/net/dev");

...
Using this hibenchmarks collector, you can monitor one or more ISC Bind servers.

The source code for this plugin in [here](https://github.com/firehol/hibenchmarks/blob/master/node.d/named.node.js).

## Example hibenchmarks charts
...
The collector (optionally) reads a configuration file named `/etc/hibenchmarks/node.d/named.conf`, with the following contents:
...
You will have to provide the config file `/etc/hibenchmarks/node.d/named.conf`, like this:

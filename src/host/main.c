...
void hibenchmarks_cleanup_and_exit(int ret) {
    ...
    info("EXIT: hibenchmarks prepares to exit with code %d...", ret);
    ...
    // unlink the pid
    if(pidfile[0]) {
        info("EXIT: removing hibenchmarks PID file '%s'...", pidfile);
        if(unlink(pidfile) != 0)
            error("EXIT: cannot unlink pidfile '%s'.", pidfile);
    }

    info("EXIT: all done - hibenchmarks is now exiting - bye bye...");
    exit(ret);
}

struct hibenchmarks_static_thread static_threads[] = {
...
void web_server_config_options(void) {
    ...
    web_allow_hibenchmarksconf_from = simple_pattern_create(config_get(CONFIG_SECTION_WEB, "allow hibenchmarks.conf from", "localhost fd* 10.* 192.168.* 172.16.* 172.17.* 172.18.* 172.19.* 172.20.* 172.21.* 172.22.* 172.23.* 172.24.* 172.25.* 172.26.* 172.27.* 172.28.* 172.29.* 172.30.* 172.31.*"), NULL, SIMPLE_PATTERN_EXACT);

#ifdef HIBENCHMARKS_WITH_ZLIB
...
void cancel_main_threads() {
    ...
    for (i = 0; static_threads[i].name != NULL ; i++) {
        if(static_threads[i].enabled == HIBENCHMARKS_MAIN_THREAD_RUNNING) {
            info("EXIT: Stopping master thread: %s", static_threads[i].name);
            hibenchmarks_thread_cancel(*static_threads[i].thread);
            found++;
        }
    }
    ...
    while(found && max > 0) {
        ...
        found = 0;
        for (i = 0; static_threads[i].name != NULL ; i++) {
            if (static_threads[i].enabled != HIBENCHMARKS_MAIN_THREAD_EXITED)
                found++;
        }
    }
...
struct option_def option_definitions[] = {
    ...
    { 't', "The internal clock of hibenchmarks.",              "seconds",     "1"},
    { 'u', "Run as user.",                                "username",    "hibenchmarks"},
    { 'v', "Print hibenchmarks version and exit.",             NULL,          NULL},
    { 'V', "Print hibenchmarks version and exit.",             NULL,          NULL},
...
int help(int exitcode) {
    ...
    fprintf(stream, "%s", "\n"
            ...
            " All rights reserved.\n"
            "\n"
            " Home Page  : https://my-hibenchmarks.io\n"
            " Source Code: https://github.com/firehol/hibenchmarks\n"
            " Wiki / Docs: https://github.com/firehol/hibenchmarks/wiki\n"
            " Support    : https://github.com/firehol/hibenchmarks/issues\n"
            " License    : https://github.com/firehol/hibenchmarks/blob/master/LICENSE.md\n"
            "\n"
            " Twitter    : https://twitter.com/linuxhibenchmarks\n"
            " Facebook   : https://www.facebook.com/linuxhibenchmarks/\n"
            "\n"
            " hibenchmarks is a https://firehol.org project.\n"
            "\n"
            "\n"
    );

    fprintf(stream, " SYNOPSIS: hibenchmarks [options]\n");
    fprintf(stream, "\n");
    ...
    fprintf(stream, "\n Advanced options:\n\n"
            ...
            "                           set hibenchmarks.conf option from the command line.\n\n"
            "  -W simple-pattern pattern string\n"
            "                           Check if string matches pattern and exit.\n\n"
    );

    fprintf(stream, "\n Signals hibenchmarks handles:\n\n"
...
void log_init(void) {
    ...
    snprintfz(filename, FILENAME_MAX, "%s/debug.log", hibenchmarks_configured_log_dir);
    stdout_filename    = config_get(CONFIG_SECTION_GLOBAL, "debug log",  filename);

    snprintfz(filename, FILENAME_MAX, "%s/error.log", hibenchmarks_configured_log_dir);
    stderr_filename    = config_get(CONFIG_SECTION_GLOBAL, "error log",  filename);

    snprintfz(filename, FILENAME_MAX, "%s/access.log", hibenchmarks_configured_log_dir);
    ...
    setenv("HIBENCHMARKS_ERRORS_THROTTLE_PERIOD", config_get(CONFIG_SECTION_GLOBAL, "errors flood protection period"    , ""), 1);
    setenv("HIBENCHMARKS_ERRORS_PER_PERIOD",      config_get(CONFIG_SECTION_GLOBAL, "errors to trigger flood protection", ""), 1);
}

static void backwards_compatible_config() {
    // allow existing configurations to work with the current version of hibenchmarks
...
static void get_hibenchmarks_configured_variables() {
    ...
    hibenchmarks_configured_hostname = config_get(CONFIG_SECTION_GLOBAL, "hostname", buf);
    debug(D_OPTIONS, "hostname set to '%s'", hibenchmarks_configured_hostname);
    ...
    hibenchmarks_configured_config_dir  = config_get(CONFIG_SECTION_GLOBAL, "config directory",    CONFIG_DIR);
    hibenchmarks_configured_log_dir     = config_get(CONFIG_SECTION_GLOBAL, "log directory",       LOG_DIR);
    hibenchmarks_configured_web_dir     = config_get(CONFIG_SECTION_GLOBAL, "web files directory", WEB_DIR);
    hibenchmarks_configured_cache_dir   = config_get(CONFIG_SECTION_GLOBAL, "cache directory",     CACHE_DIR);
    hibenchmarks_configured_varlib_dir  = config_get(CONFIG_SECTION_GLOBAL, "lib directory",       VARLIB_DIR);
    hibenchmarks_configured_home_dir    = config_get(CONFIG_SECTION_GLOBAL, "home directory",      CACHE_DIR);

    {
        char plugins_dirs[(FILENAME_MAX * 2) + 1];
        snprintfz(plugins_dirs, FILENAME_MAX * 2, "\"%s\" \"%s/custom-plugins.d\"", PLUGINS_DIR, CONFIG_DIR);
        hibenchmarks_configured_plugins_dir_base = strdupz(config_get(CONFIG_SECTION_GLOBAL, "plugins directory",  plugins_dirs));
        quoted_strings_splitter(hibenchmarks_configured_plugins_dir_base, plugin_directories, PLUGINSD_MAX_DIRECTORIES, config_isspace);
        hibenchmarks_configured_plugins_dir = plugin_directories[0];
    }
    ...
    hibenchmarks_configured_host_prefix = config_get(CONFIG_SECTION_GLOBAL, "host access prefix", "");
    verify_hibenchmarks_host_prefix();
...
static void get_system_timezone(void) {
    ...
    hibenchmarks_configured_timezone = config_get(CONFIG_SECTION_GLOBAL, "timezone", timezone);
}
...
void set_global_environment() {
    {
        ...
        setenv("HIBENCHMARKS_UPDATE_EVERY", b, 1);
    }

    setenv("HIBENCHMARKS_HOSTNAME"   , hibenchmarks_configured_hostname, 1);
    setenv("HIBENCHMARKS_CONFIG_DIR" , verify_required_directory(hibenchmarks_configured_config_dir),  1);
    setenv("HIBENCHMARKS_PLUGINS_DIR", verify_required_directory(hibenchmarks_configured_plugins_dir), 1);
    setenv("HIBENCHMARKS_WEB_DIR"    , verify_required_directory(hibenchmarks_configured_web_dir),     1);
    setenv("HIBENCHMARKS_CACHE_DIR"  , verify_required_directory(hibenchmarks_configured_cache_dir),   1);
    setenv("HIBENCHMARKS_LIB_DIR"    , verify_required_directory(hibenchmarks_configured_varlib_dir),  1);
    setenv("HIBENCHMARKS_LOG_DIR"    , verify_required_directory(hibenchmarks_configured_log_dir),     1);
    setenv("HOME"               , verify_required_directory(hibenchmarks_configured_home_dir),    1);
    setenv("HIBENCHMARKS_HOST_PREFIX", hibenchmarks_configured_host_prefix, 1);
...
int main(int argc, char **argv) {
                ...
                case 'W':
                    {
                        ...
                        if(strcmp(optarg, "unittest") == 0) {
                            ...
                            get_hibenchmarks_configured_variables();
                            default_rrd_update_every = 1;
                            default_rrd_memory_mode = RRD_MEMORY_MODE_RAM;
                            ...
                            if(optind + 3 > argc) {
                                fprintf(stderr, "%s", "\nUSAGE: -W set 'section' 'key' 'value'\n\n"
                                        " Overwrites settings of hibenchmarks.conf.\n"
                                        "\n"
                                        " These options interact with: -c hibenchmarks.conf\n"
                                        " If -c hibenchmarks.conf is given on the command line,\n"
                                        " before -W set... the user may overwrite command\n"
                                        " line parameters at hibenchmarks.conf\n"
                                        " If -c hibenchmarks.conf is given after (or missing)\n"
                            ...
                            // so the caller can use -c hibenchmarks.conf before or
                            // after this parameter to prevent or allow overwriting
                            // variables at hibenchmarks.conf
                            ...
                            if(optind + 3 > argc) {
                                fprintf(stderr, "%s", "\nUSAGE: -W get 'section' 'key' 'value'\n\n"
                                        " Prints settings of hibenchmarks.conf.\n"
                                        "\n"
                                        " These options interact with: -c hibenchmarks.conf\n"
                                        " -c hibenchmarks.conf has to be given before -W get.\n"
                                        "\n"
                                );
                                return 1;
                            }
    ...
    // initialize hibenchmarks
    {
        ...
        i = (int)config_get_number(CONFIG_SECTION_GLOBAL, "glibc malloc arena max for hibenchmarks", 1);
        if(i > 0)
        ...
        if(chdir(hibenchmarks_configured_config_dir) == -1)
            fatal("Cannot cd to '%s'", hibenchmarks_configured_config_dir);
    }
...

    {
        ...
        setenv("HIBENCHMARKS_DEBUG_FLAGS", flags, 1);
        ...
        {
            char filename[FILENAME_MAX + 1];
            snprintfz(filename, FILENAME_MAX, "%s/stream.conf", hibenchmarks_configured_config_dir);
        ...
        default_stacksize = hibenchmarks_threads_init();
        ...
        for (i = 0; static_threads[i].name != NULL ; i++) {
            struct hibenchmarks_static_thread *st = &static_threads[i];
        ...
        if(getuid() == 0) {
            user = config_get(CONFIG_SECTION_GLOBAL, "run as user", HIBENCHMARKS_USER);
        }
        else {
    ...
    info("hibenchmarks started on pid %d.", getpid());
    ...
    hibenchmarks_threads_init_after_fork((size_t)config_get_number(CONFIG_SECTION_GLOBAL, "pthread stack size", (long)default_stacksize));

    // ------------------------------------------------------------------------
    // initialize rrd, registry, health, rrdpush, etc.

    rrd_init(hibenchmarks_configured_hostname);
    ...
    for (i = 0; static_threads[i].name != NULL ; i++) {
        ...
        if(st->enabled) {
            st->thread = mallocz(sizeof(hibenchmarks_thread_t));
            debug(D_SYSTEM, "Starting thread %s.", st->name);
            hibenchmarks_thread_create(st->thread, st->name, HIBENCHMARKS_THREAD_OPTION_DEFAULT, st->start_routine, st);
        }
        else debug(D_SYSTEM, "Not starting thread %s.", st->name);
    }

    info("hibenchmarks initialization completed. Enjoy real-time performance monitoring!");

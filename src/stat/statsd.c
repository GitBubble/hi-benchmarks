...
#define STATSD_FIRST_PTR_MUTEX hibenchmarks_mutex_t first_mutex
#define STATSD_FIRST_PTR_MUTEX_INIT .first_mutex = HIBENCHMARKS_MUTEX_INITIALIZER
#define STATSD_FIRST_PTR_MUTEX_LOCK(index) hibenchmarks_mutex_lock(&((index)->first_mutex))
#define STATSD_FIRST_PTR_MUTEX_UNLOCK(index) hibenchmarks_mutex_unlock(&((index)->first_mutex))
...
typedef struct statsd_metric {
    ...
    collected_number last;          // the last value sent to hibenchmarks
...
static inline STATSD_METRIC *statsd_find_or_add_metric(STATSD_INDEX *index, const char *name, STATSD_METRIC_TYPE type) {
    ...
    if(unlikely(!m)) {
        ...
        if(type == STATSD_METRIC_TYPE_HISTOGRAM || type == STATSD_METRIC_TYPE_TIMER) {
            ...
            hibenchmarks_mutex_init(&m->histogram.ext->mutex);
        }
    ...
    if(unlikely(value_is_zinit(value))) {
        ...
        while(samples-- > 0) {
            ...
            if(unlikely(m->histogram.ext->used == m->histogram.ext->size)) {
                hibenchmarks_mutex_lock(&m->histogram.ext->mutex);
                ...
                hibenchmarks_mutex_unlock(&m->histogram.ext->mutex);
            }
...
void *statsd_collector_thread(void *ptr) {
    ...
    hibenchmarks_thread_cleanup_push(statsd_collector_thread_cleanup, d);
    ...
    hibenchmarks_thread_cleanup_pop(1);
    return NULL;
}
    ...
    while(fgets(buffer, STATSD_CONF_LINE_MAX, fp) != NULL) {
        ...
        *value = '\0';
        ...
        if(unlikely(dict)) {
            ...
            if(!strcmp(name, "name")) {
                freez((void *)app->name);
                hibenchmarks_fix_chart_name(value);
            ...
            if(!strcmp(name, "name")) {
                ...
                hibenchmarks_fix_chart_id(value);
...
// --------------------------------------------------------------------------------------------------------------------
// send metrics to hibenchmarks - in private charts - called from the main thread
...
static inline void statsd_get_metric_type_and_id(STATSD_METRIC *m, char *type, char *id, const char *defid, size_t len) {
    ...
    hibenchmarks_fix_chart_id(type);
    hibenchmarks_fix_chart_id(id);
}
...
static inline RRDSET *statsd_private_rrdset_create(
    ...
    if(unlikely(statsd.private_charts >= statsd.max_private_charts)) {
        ...
        info("STATSD: metric '%s' will be charted with memory mode = none, because the maximum number of charts (%zu) has been reached. Increase the number of charts by editing hibenchmarks.conf, [statsd] section.", m->name, statsd.max_private_charts);
    ...
    for(m = index->first; m ; m = m->next) {
        ...
        if(unlikely(!(m->options & STATSD_METRIC_OPTION_PRIVATE_CHART_CHECKED))) {
            if(unlikely(statsd.private_charts >= statsd.max_private_charts_hard)) {
                ...
                info("STATSD: metric '%s' will not be charted, because the hard limit of the maximum number of charts (%zu) has been reached. Increase the number of charts by editing hibenchmarks.conf, [statsd] section.", m->name, statsd.max_private_charts);
                m->options &= ~STATSD_METRIC_OPTION_PRIVATE_CHART_ENABLED;
            }
            else {
...
static void statsd_main_cleanup(void *data) {
    struct hibenchmarks_static_thread *static_thread = (struct hibenchmarks_static_thread *)data;
    static_thread->enabled = HIBENCHMARKS_MAIN_THREAD_EXITING;
    info("cleaning up...");
    ...
    if (statsd.collection_threads_status) {
        ...
        for (i = 0; i < statsd.threads; i++) {
            if(statsd.collection_threads_status[i].status) {
                ...
                hibenchmarks_thread_cancel(statsd.collection_threads_status[i].thread);
            }
            else {
    ...
    static_thread->enabled = HIBENCHMARKS_MAIN_THREAD_EXITED;
}

void *statsd_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(statsd_main_cleanup, ptr);
    ...
    if(statsd.update_every < default_rrd_update_every) {
        error("STATSD: minimum flush interval %d given, but the minimum is the update every of hibenchmarks. Using %d", statsd.update_every, default_rrd_update_every);
        statsd.update_every = default_rrd_update_every;
    }
    ...
    // read custom application definitions
    {
        ...
        snprintfz(filename, FILENAME_MAX, "%s/statsd.d", hibenchmarks_configured_config_dir);
    ...
    for(i = 0; i < statsd.threads ;i++) {
        ...
        char tag[HIBENCHMARKS_THREAD_TAG_MAX + 1];
        snprintfz(tag, HIBENCHMARKS_THREAD_TAG_MAX, "STATSD_COLLECTOR[%d]", i + 1);
        hibenchmarks_thread_create(&statsd.collection_threads_status[i].thread, tag, HIBENCHMARKS_THREAD_OPTION_DEFAULT, statsd_collector_thread, &statsd.collection_threads_status[i]);
    }
    ...
    RRDSET *st_metrics = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Metrics in the hibenchmarks statsd database"
            , "metrics"
            , "statsd"
            , "stats"
            , 132010
    ...
    RRDSET *st_useful_metrics = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Useful metrics in the hibenchmarks statsd database"
            , "metrics"
            , "statsd"
            , "stats"
            , 132010
    ...
    RRDSET *st_events = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Events processed by the hibenchmarks statsd server"
            , "events/s"
            , "statsd"
            , "stats"
            , 132011
    ...
    RRDSET *st_reads = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Read operations made by the hibenchmarks statsd server"
            , "reads/s"
            , "statsd"
            , "stats"
            , 132012
    ...
    RRDSET *st_bytes = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Bytes read by the hibenchmarks statsd server"
            , "kilobits/s"
            , "hibenchmarks"
            , "stats"
            , 132013
    ...
    RRDSET *st_packets = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Network packets processed by the hibenchmarks statsd server"
            , "packets/s"
            , "hibenchmarks"
            , "stats"
            , 132014
    ...
    RRDSET *st_pcharts = rrdset_create_localhost(
            ...
            , "statsd"
            , NULL
            , "Private metric charts created by the hibenchmarks statsd server"
            , "charts"
            , "statsd"
            , "stats"
            , 132020

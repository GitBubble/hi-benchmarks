...
int mysendfile(struct web_client *w, char *filename) {
    debug(D_WEB_CLIENT, "%llu: Looking for file '%s/%s'", w->id, hibenchmarks_configured_web_dir, filename);
    ...
    snprintfz(webfilename, FILENAME_MAX, "%s/%s", hibenchmarks_configured_web_dir, filename);
    ...
    while(!done) {
        ...
        if ((statbuf.st_mode & S_IFMT) == S_IFDIR) {
            snprintfz(webfilename, FILENAME_MAX, "%s/%s/index.html", hibenchmarks_configured_web_dir, filename);
            continue;
        }
...
static inline char *http_header_parse(struct web_client *w, char *s, int parse_useragent) {
    ...
    *ve = '\0';
    ...
    if(hash == hash_origin && !strcasecmp(s, "Origin"))
        strncpyz(w->origin, v, HIBENCHMARKS_WEB_REQUEST_ORIGIN_HEADER_SIZE);
    ...
    while(*s) {
        ...
        if(likely(*s++ == '\n')) {
            ...
            if(unlikely(*s == '\r' && s[1] == '\n')) {
                ...
                *ue = '\0';
                url_decode_r(w->decoded_url, encoded_url, HIBENCHMARKS_WEB_REQUEST_URL_SIZE + 1);
                *ue = ' ';
                
                // copy the URL - we are going to overwrite parts of it
                // FIXME -- we should avoid it
                strncpyz(w->last_url, w->decoded_url, HIBENCHMARKS_WEB_REQUEST_URL_SIZE);
...
static inline void web_client_send_http_header(struct web_client *w) {
    ...
    buffer_sprintf(w->response.header_output,
            "HTTP/1.1 %d %s\r\n"
                    "Connection: %s\r\n"
                    "Server: HiBenchmarks Embedded HTTP Server v%s\r\n"
...
static inline int web_client_switch_host(RRDHOST *host, struct web_client *w, char *url) {
    ...
    if(tok && *tok) {
        ...
        w->last_url[0] = '/';
        if(url && *url) strncpyz(&w->last_url[1], url, HIBENCHMARKS_WEB_REQUEST_URL_SIZE - 1);
        else w->last_url[1] = '\0';
    ...
    buffer_flush(w->response.data);
    w->response.data->contenttype = CT_TEXT_HTML;
    buffer_strcat(w->response.data, "This hibenchmarks does not maintain a database for host: ");
...
static inline int web_client_process_url(RRDHOST *host, struct web_client *w, char *url) {
    ...
    if(unlikely(!hash_api)) {
        ...
        hash_hibenchmarks_conf = simple_hash("hibenchmarks.conf");
    ...
    if(likely(tok && *tok)) {
        ...
        else if(unlikely(hash == hash_hibenchmarks_conf && strcmp(tok, "hibenchmarks.conf") == 0)) {    // hibenchmarks.conf
            if(unlikely(!web_client_can_access_hibenchmarksconf(w)))
                return web_client_permission_denied(w);

            debug(D_WEB_CLIENT_ACCESS, "%llu: generating hibenchmarks.conf ...", w->id);
            w->response.data->contenttype = CT_TEXT_PLAIN;
            buffer_flush(w->response.data);
...
void web_client_process_request(struct web_client *w) {
    ...
    switch(http_request_validate(w)) {
        ...
        case HTTP_VALIDATION_INCOMPLETE:
            if(w->response.data->len > HIBENCHMARKS_WEB_REQUEST_MAX_SIZE) {
...
ssize_t web_client_send_deflate(struct web_client *w)
{
    ...
    if(w->response.zhave == w->response.zsent) {
        ...
        w->response.zstream.next_out = w->response.zbuffer;
        w->response.zstream.avail_out = HIBENCHMARKS_WEB_RESPONSE_ZLIB_CHUNK_SIZE;
        ...
        w->response.zhave = HIBENCHMARKS_WEB_RESPONSE_ZLIB_CHUNK_SIZE - w->response.zstream.avail_out;
        w->response.zsent = 0;
...
ssize_t web_client_receive(struct web_client *w)
{
    ...
    buffer_need_bytes(w->response.data, HIBENCHMARKS_WEB_REQUEST_RECEIVE_SIZE);

...
int do_proc_vmstat(int update_every, usec_t dt) {
    ...
    if(unlikely(!ff)) {
        ...
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/vmstat");
    ...
    if(do_pgfaults) {
        ...
        if(unlikely(!st_pgfaults)) {
            st_pgfaults = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "vmstat"
                    , HIBENCHMARKS_CHART_PRIO_MEM_SYSTEM_PGFAULTS
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_numa == CONFIG_BOOLEAN_YES || (do_numa == CONFIG_BOOLEAN_AUTO && has_numa)) {
        ...
        if(unlikely(!st_numa)) {
            st_numa = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "vmstat"
                    , HIBENCHMARKS_CHART_PRIO_MEM_NUMA
                    , update_every
                    , RRDSET_TYPE_LINE
            );

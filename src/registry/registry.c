...
static inline void registry_lock(void) {
    hibenchmarks_mutex_lock(&registry.lock);
}

static inline void registry_unlock(void) {
    hibenchmarks_mutex_unlock(&registry.lock);
}
...
static void registry_set_cookie(struct web_client *w, const char *guid) {
    ...
    snprintfz(w->cookie1, HIBENCHMARKS_WEB_REQUEST_COOKIE_SIZE, HIBENCHMARKS_REGISTRY_COOKIE_NAME "=%s; Expires=%s", guid, edate);

    if(registry.registry_domain && registry.registry_domain[0])
        snprintfz(w->cookie2, HIBENCHMARKS_WEB_REQUEST_COOKIE_SIZE, HIBENCHMARKS_REGISTRY_COOKIE_NAME "=%s; Domain=%s; Expires=%s", guid, registry.registry_domain, edate);
}
...
// the main method for searching the URLs of a hibenchmarks

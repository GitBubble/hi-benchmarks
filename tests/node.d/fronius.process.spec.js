...
var hibenchmarks = require("../../node.d/node_modules/hibenchmarks");
...
describe("fronius main processing", function () {
    ...
    beforeAll(function () {
        ...
        hibenchmarks.options.DEBUG = false;
    });
    ...
    it("should send parsed values to hibenchmarks", function () {
        hibenchmarks.send = jasmine.createSpy("send");

        subject.processResponse(service, exampleResponse);

        expect(hibenchmarks.send.calls.count()).toBe(6);

        // check if some parsed values were sent.
        var powerChart = hibenchmarks.send.calls.argsFor(5)[0];

        expect(powerChart).toContain("SET p_grid = -3431");
        expect(powerChart).toContain("SET p_pv = 4341");

        var inverterChart = hibenchmarks.send.calls.argsFor(0)[0];

        expect(inverterChart).toContain("SET 1 = 4341");

        var autonomyChart = hibenchmarks.send.calls.argsFor(3)[0];

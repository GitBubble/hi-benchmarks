...
inline char *health_config_dir(void) {
    ...
    snprintfz(buffer, FILENAME_MAX, "%s/health.d", hibenchmarks_configured_config_dir);
...
static inline void health_alarm_log_process(RRDHOST *host) {
    ...
    hibenchmarks_rwlock_rdlock(&host->health_log.alarm_log_rwlock);
    ...
    hibenchmarks_rwlock_unlock(&host->health_log.alarm_log_rwlock);

    if(host->health_log.count <= host->health_log.max)
        return;

    // cleanup excess entries in the log
    hibenchmarks_rwlock_wrlock(&host->health_log.alarm_log_rwlock);
...
void *health_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(health_main_cleanup, ptr);
    ...
    while(!hibenchmarks_exit) {
        ...
        rrdhost_foreach_read(host) {
            ...
            if(unlikely(runnable && !hibenchmarks_exit)) {
                rrdhost_rdlock(host);

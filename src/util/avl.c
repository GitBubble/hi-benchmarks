...
void avl_read_lock(avl_tree_lock *t) {
    ...
    hibenchmarks_mutex_lock(&t->mutex);
#else
    hibenchmarks_rwlock_rdlock(&t->rwlock);
#endif
...
void avl_write_lock(avl_tree_lock *t) {
    ...
    hibenchmarks_mutex_lock(&t->mutex);
#else
    hibenchmarks_rwlock_wrlock(&t->rwlock);
#endif
#endif /* AVL_WITHOUT_PTHREADS */
}
...
#ifndef AVL_WITHOUT_PTHREADS
#ifdef AVL_LOCK_WITH_MUTEX
    hibenchmarks_mutex_unlock(&t->mutex);
#else
    hibenchmarks_rwlock_unlock(&t->rwlock);
#endif
#endif /* AVL_WITHOUT_PTHREADS */
}
...
#ifdef AVL_LOCK_WITH_MUTEX
    lock = hibenchmarks_mutex_init(&t->mutex, NULL);
#else
    lock = hibenchmarks_rwlock_init(&t->rwlock);
#endif

...
int become_user(const char *username, int pid_fd) {
    ...
    create_needed_dir(hibenchmarks_configured_cache_dir, uid, gid);
    create_needed_dir(hibenchmarks_configured_varlib_dir, uid, gid);
...
static void oom_score_adj(void) {
    ...
    // check hibenchmarks.conf configuration
    ...
    if(fd != -1) {
        ...
        if(written) {
            ...
            error("Failed to adjust my Out-Of-Memory (OOM) score to %d. Running with %d. (systemd systems may change it via hibenchmarks.service)", (int)wanted_score, (int)old_score);
    }
    else
...
static void process_nice_level(void) {
    ...
    if(nice(nice_level) == -1) error("Cannot set hibenchmarks CPU nice level to %d.", nice_level);
    else debug(D_SYSTEM, "Set hibenchmarks nice level to %d.", nice_level);
#endif // HAVE_NICE
};
...
struct sched_def {
    ...
    uint8_t flags;
        ...
        // the first defined is the default used by hibenchmarks

        // the available members are important too!
        // these are all the possible scheduling policies supported by hibenchmarks
...
static void sched_setscheduler_set(void) {
    ...
    if(scheduler_defaults[0].name) {
        ...
        if(i != 0) {
            error("Cannot adjust hibenchmarks scheduling policy to %s (%d), with priority %d. Falling back to nice.", name, policy, priority);
        }
        else {
            info("Adjusted hibenchmarks scheduling policy to %s (%d), with priority %d.", name, policy, priority);
...
int become_daemon(int dont_fork, const char *user)
{
    ...
    if(user && *user) {
        ...
        create_needed_dir(hibenchmarks_configured_cache_dir, getuid(), getgid());
        create_needed_dir(hibenchmarks_configured_varlib_dir, getuid(), getgid());
    }

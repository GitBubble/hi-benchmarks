...
BACKEND_HOST="${3}"
BACKEND_PORT="${4}"
FILE="${HIBENCHMARKS_NC_BACKEND_DIR-/tmp}/hibenchmarks-nc-backend-${MY_PORT}"

log() {
	logger --stderr --id=$$ --tag "hibenchmarks-nc-backend" "${*}"
}
...
Usage:
    ...
    PORT          The port this script will listen
                  (configure hibenchmarks to use this as a second backend)

    BACKEND_HOST  The real backend host
    BACKEND_PORT  The real backend port

    This script can act as fallback backend for hibenchmarks.
    It will receive metrics from hibenchmarks, save them to
    ${FILE}
    and once hibenchmarks reconnects to the real-backend, this script
    will push all metrics collected to the real-backend too and
    wait for a failure to happen again.

    Only one hibenchmarks can connect to this script at a time.
    If you need fallback for multiple hibenchmarks, run this script

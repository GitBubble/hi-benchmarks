...
inline size_t pluginsd_process(RRDHOST *host, struct plugind *cd, FILE *fp, int trust_durations) {
    ...
    while(!ferror(fp)) {
        ...
        if(likely(!simple_hash_strcmp(s, "SET", &hash))) {
            ...
            error("sent command '%s' which is not known by hibenchmarks, for host '%s'. Disabling it.", s, host->hostname);
            enabled = 0;
            break;
        }
    }
...
void *pluginsd_worker_thread(void *arg) {
    hibenchmarks_thread_cleanup_push(pluginsd_worker_thread_cleanup, arg);
...
static void pluginsd_main_cleanup(void *data) {
    ...
    for (cd = pluginsd_root; cd; cd = cd->next) {
        if (cd->enabled && !cd->obsolete) {
            ...
            hibenchmarks_thread_cancel(cd->thread);
        }
    }
...
void *pluginsd_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(pluginsd_main_cleanup, ptr);
            ...
            while(likely((file = readdir(dir)))) {
                ...
                if(unlikely(!cd)) {
                    ...
                    if(cd->enabled) {
                        ...
                        snprintfz(tag, HIBENCHMARKS_THREAD_TAG_MAX, "PLUGINSD[%s]", pluginname);
                        // spawn a new thread for it
                        hibenchmarks_thread_create(&cd->thread, tag, HIBENCHMARKS_THREAD_OPTION_DEFAULT, pluginsd_worker_thread, cd);
                    }
                }
            }

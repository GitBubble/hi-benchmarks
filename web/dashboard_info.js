...
var hibenchmarksDashboard = window.hibenchmarksDashboard || {};
...
hibenchmarksDashboard.menu = {
    ...
    'services': {
        ...
        info: 'Resources utilization of systemd services. hibenchmarks monitors all systemd services via cgroups (the resources accounting used by containers). '
    },
    ...
    'apps': {
        ...
        info: 'Per application statistics are collected using hibenchmarks\'s <code>apps.plugin</code>. This plugin walks through all processes and aggregates statistics for applications of interest, defined in <code>/etc/hibenchmarks/apps_groups.conf</code> (the default is <a href="https://github.com/firehol/hibenchmarks/blob/master/conf.d/apps_groups.conf" target="_blank">here</a>). The plugin internally builds a process tree (much like <code>ps fax</code> does), and groups processes together (evaluating both child and parent processes) so that the result is always a chart with a predefined set of dimensions (of course, only application groups found running are reported). The reported values are compatible with <code>top</code>, although the hibenchmarks plugin counts also the resources of exited children (unlike <code>top</code> which shows only the resources of the currently running processes). So for processes like shell scripts, the reported values include the resources used by the commands these scripts run within each timeframe.',
        height: 1.5
    },
    ...
    'users': {
        ...
        info: 'Per user statistics are collected using hibenchmarks\'s <code>apps.plugin</code>. This plugin walks through all processes and aggregates statistics per user. The reported values are compatible with <code>top</code>, although the hibenchmarks plugin counts also the resources of exited children (unlike <code>top</code> which shows only the resources of the currently running processes). So for processes like shell scripts, the reported values include the resources used by the commands these scripts run within each timeframe.',
        height: 1.5
    },
    ...
    'groups': {
        ...
        info: 'Per user group statistics are collected using hibenchmarks\'s <code>apps.plugin</code>. This plugin walks through all processes and aggregates statistics per user group. The reported values are compatible with <code>top</code>, although the hibenchmarks plugin counts also the resources of exited children (unlike <code>top</code> which shows only the resources of the currently running processes). So for processes like shell scripts, the reported values include the resources used by the commands these scripts run within each timeframe.',
        height: 1.5
    },
    ...
    'fping': {
        ...
        info: 'Network latency statistics, via <b>fping</b>. <b>fping</b> is a program to send ICMP echo probes to network hosts, similar to <code>ping</code>, but much better performing when pinging multiple hosts. fping versions after 3.15 can be directly used as hibenchmarks plugins.'
    },
...
hibenchmarksDashboard.submenu = {
    ...
    'web_log.urls': {
        info: 'Number of requests for each <code>URL pattern</code> defined in <a href="https://github.com/firehol/hibenchmarks/blob/master/conf.d/python.d/web_log.conf" target="_blank"><code>/etc/hibenchmarks/python.d/web_log.conf</code></a>. This chart counts all requests matching the URL patterns defined, independently of the web server response codes (i.e. both successful and unsuccessful).'
    },
...
hibenchmarksDashboard.context = {
    'system.cpu': {
        info: function(os) {
            ...
            return 'Total CPU utilization (all cores). 100% here means there is no CPU idle time at all. You can get per core usage at the <a href="#menu_cpu">CPUs</a> section and per application usage at the <a href="#menu_apps">Applications Monitoring</a> section.'
                + hibenchmarksDashboard.sparkline('<br/>Keep an eye on <b>iowait</b> ', 'system.cpu', 'iowait', '%', '. If it is constantly high, your disks are a bottleneck and they slow your system down.')
                + hibenchmarksDashboard.sparkline('<br/>An important metric worth monitoring, is <b>softirq</b> ', 'system.cpu', 'softirq', '%', '. A constantly high percentage of softirq may indicate network driver issues.');
        },
        valueRange: "[0, 100]"
    },
    ...
    'system.swapio': {
        info: 'Total Swap I/O. (hibenchmarks measures both <code>in</code> and <code>out</code>. If either of them is not shown in the chart, it is because it is zero - you can change the page settings to always render all the available dimensions on all charts).'
    },
    ...
    'system.idlejitter': {
        info: 'Idle jitter is calculated by hibenchmarks. A thread is spawned that requests to sleep for a few microseconds. When the system wakes it up, it measures how many microseconds have passed. The difference between the requested and the actual duration of the sleep, is the <b>idle jitter</b>. This number is useful in real-time environments, where CPU jitter can affect the quality of the service (like VoIP media gateways).'
    },
    ...
    'mem.ksm_savings': {
        heads: [
            hibenchmarksDashboard.gaugeChart('Saved', '12%', 'savings', '#0099CC')
        ]
    },
    ...
    'mem.ksm_ratios': {
        heads: [
            function(os, id) {
                void(os);
                return  '<div data-hibenchmarks="' + id + '"'
    ...
    'apps.vmem': {
        info: 'Virtual memory allocated by applications. Please check <a href="https://github.com/firehol/hibenchmarks/wiki/hibenchmarks-virtual-memory-size" target="_blank">this article</a> for more information.'
    },
    ...
    'users.vmem': {
        info: 'Virtual memory allocated per user. Please check <a href="https://github.com/firehol/hibenchmarks/wiki/hibenchmarks-virtual-memory-size" target="_blank">this article</a> for more information.'
    },
    ...
    'groups.vmem': {
        info: 'Virtual memory allocated per user group. Please check <a href="https://github.com/firehol/hibenchmarks/wiki/hibenchmarks-virtual-memory-size" target="_blank">this article</a> for more information.'
    },
    ...
    'tc.qos': {
        heads: [
            function(os, id) {
                ...
                if(id.match(/.*-ifb$/))
                    return hibenchmarksDashboard.gaugeChart('Inbound', '12%', '', '#5555AA');
                else
                    return hibenchmarksDashboard.gaugeChart('Outbound', '12%', '', '#AA9900');
            }
        ]
    },
    ...
    'net.net': {
        mainheads: [
            function(os, id) {
                ...
                if(id.match(/^cgroup_.*/)) {
                    ...
                    return hibenchmarksDashboard.gaugeChart('Received' + iface, '12%', 'received');
                }
                else
                    return '';
            },
            function(os, id) {
                ...
                if(id.match(/^cgroup_.*/)) {
                    ...
                    return hibenchmarksDashboard.gaugeChart('Sent' + iface, '12%', 'sent');
                }
                else
                    return '';
            }
        ...
        heads: [
            function(os, id) {
                void(os);
                if(!id.match(/^cgroup_.*/))
                    return hibenchmarksDashboard.gaugeChart('Received', '12%', 'received');
                else
                    return '';
            },
            function(os, id) {
                void(os);
                if(!id.match(/^cgroup_.*/))
                    return hibenchmarksDashboard.gaugeChart('Sent', '12%', 'sent');
                else
                    return '';
            }
        ]
    ...
    'netfilter.sockets': {
        colors: '#88AA00',
        heads: [
            hibenchmarksDashboard.gaugeChart('Active Connections', '12%', '', '#88AA00')
        ]
    },

    'netfilter.new': {
        heads: [
            hibenchmarksDashboard.gaugeChart('New Connections', '12%', 'new', '#5555AA')
        ]
    },
    ...
    'disk.util': {
        colors: '#FF5588',
        heads: [
            hibenchmarksDashboard.gaugeChart('Utilization', '12%', '', '#FF5588')
        ],
    ...
    'disk.io': {
        heads: [
            hibenchmarksDashboard.gaugeChart('Read', '12%', 'reads'),
            hibenchmarksDashboard.gaugeChart('Write', '12%', 'writes')
        ],
    ...
    'apache.connections': {
        colors: HIBENCHMARKS.colors[4],
        mainheads: [
            hibenchmarksDashboard.gaugeChart('Connections', '12%', '', HIBENCHMARKS.colors[4])
        ]
    },

    'apache.requests': {
        colors: HIBENCHMARKS.colors[0],
        mainheads: [
            hibenchmarksDashboard.gaugeChart('Requests', '12%', '', HIBENCHMARKS.colors[0])
        ]
    },
    ...
    'apache.net': {
        colors: HIBENCHMARKS.colors[3],
        mainheads: [
            hibenchmarksDashboard.gaugeChart('Bandwidth', '12%', '', HIBENCHMARKS.colors[3])
        ]
    },
    ...
    'hibenchmarks.response_time': {
        info: 'The hibenchmarks API response time measures the time hibenchmarks needed to serve requests. This time includes everything, from the reception of the first byte of a request, to the dispatch of the last byte of its reply, therefore it includes all network latencies involved (i.e. a client over a slow network will influence these metrics).'
    },
    ...
    'retroshare.bandwidth': {
        ...
        mainheads: [
            hibenchmarksDashboard.gaugeChart('Received', '12%', 'bandwidth_down_kb'),
            hibenchmarksDashboard.gaugeChart('Sent', '12%', 'bandwidth_up_kb')
        ]
    },
        ...
        mainheads: [
            function(os, id) {
                ...
                return  '<div data-hibenchmarks="' + id + '"'
                    ...
                    + ' data-after="-CHART_DURATION"'
                    + ' data-points="CHART_DURATION"'
                    + ' data-colors="' + HIBENCHMARKS.colors[4] + '"'
                    + ' role="application"></div>';
            }
        ]
    },
        ...
        mainheads: [
            function(os, id) {
                ...
                return  '<div data-hibenchmarks="' + id + '"'
                    ...
                    + ' data-after="-CHART_DURATION"'
                    + ' data-points="CHART_DURATION"'
                    + ' data-colors="' + HIBENCHMARKS.colors[1] + '"'
                    + ' role="application"></div>';
            }
        ]
    },
        ...
        mainheads: [
            function(os, id) {
                ...
                return  '<div data-hibenchmarks="' + id + '"'
                    ...
                    + ' data-after="-CHART_DURATION"'
                    + ' data-points="CHART_DURATION"'
                    + ' data-colors="' + HIBENCHMARKS.colors[2] + '"'
                    + ' role="application"></div>';
            },
            function(os, id) {
                void(os);
                return  '<div data-hibenchmarks="' + id + '"'
                    ...
                    + ' data-after="-CHART_DURATION"'
                    + ' data-points="CHART_DURATION"'
                    + ' data-colors="' + HIBENCHMARKS.colors[3] + '"'
                    + ' role="application"></div>';
            }
        ]
    },
        ...
        mainheads: [
            function(os, id) {
                ...
                return  '<div data-hibenchmarks="' + id + '"'
                    ...
                    + ' data-points="CHART_DURATION"'
                    + ' data-common-max="' + id + '"'
                    + ' data-colors="' + HIBENCHMARKS.colors[0] + '"'
    ...
    'web_log.clients_all': {
        info: 'Unique client IPs accessing the web server since the last restart of hibenchmarks. This plugin keeps in memory all the unique IPs that have accessed the web server. On very busy web servers (several millions of unique IPs) you may want to disable this chart (check <a href="https://github.com/firehol/hibenchmarks/blob/master/conf.d/python.d/web_log.conf" target="_blank"><code>/etc/hibenchmarks/python.d/web_log.conf</code></a>).'
    },
    ...
    'web_log.squid_clients_all': {
        info: 'Unique client IPs accessing squid since the last restart of hibenchmarks. This plugin keeps in memory all the unique IPs that have accessed the server. On very busy squid servers (several millions of unique IPs) you may want to disable this chart (check <a href="https://github.com/firehol/hibenchmarks/blob/master/conf.d/python.d/web_log.conf" target="_blank"><code>/etc/hibenchmarks/python.d/web_log.conf</code></a>).'
    },

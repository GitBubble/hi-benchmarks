...
static int nfstat_collect_conntrack() {
    ...
    if (ret == -1) {
        error("NFSTAT: error communicating with kernel. This plugin can only work when hibenchmarks runs as root.");
        return 1;
    }
...
static int nfacct_collect() {
    ...
    // verify we run without issues
    if (ret == -1) {
        error("NFACCT: error communicating with kernel. This plugin can only work when hibenchmarks runs as root.");
        return 1;
    }
...
void *nfacct_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(nfacct_main_cleanup, ptr);

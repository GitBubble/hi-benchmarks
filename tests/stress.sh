...
echo "using hibenchmarks server at: $host"

charts=($(curl "$host/hibenchmarks.conf" 2>/dev/null | grep "^\[" | cut -d '[' -f 2 | cut -d ']' -f 1 | grep -v ^global$ | grep -v "^plugin" | sort -u))
if [ "${#charts[@]}" -eq 0 ]
	then
...
update_every="$(curl "$host/hibenchmarks.conf" 2>/dev/null | grep "update every = " | head -n 1 | cut -d '=' -f 2)"
[ $(( update_every + 1 - 1)) -eq 0 ] && update_every=1

entries="$(curl "$host/hibenchmarks.conf" 2>/dev/null | grep "history = " | head -n 1 | cut -d '=' -f 2)"
...
file="$(mktemp /tmp/hibenchmarks-stress-XXXXXXXX)"
cleanup() {
	echo "cleanup"
	[ -f $file ] && rm $file
}

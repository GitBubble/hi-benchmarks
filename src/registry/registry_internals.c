...
static inline int is_machine_guid_blacklisted(const char *guid) {
    ...
    // we blacklist them here, so that the next version of hibenchmarks will generate
    // new ones.
...
char *registry_get_this_machine_guid(void) {
    ...
    if(fd != -1) {
        ...
        if(read(fd, buf, GUID_LEN) != GUID_LEN)
            ...
            if(regenerate_guid(buf, guid) == -1) {
                error("Failed to validate machine GUID '%s' from '%s'. Ignoring it - this might mean this hibenchmarks will appear as duplicate in the registry.",
    ...
    setenv("HIBENCHMARKS_REGISTRY_UNIQUE_ID", guid, 1);

...
unsigned long test_hibenchmarks_internal(void) {
...
int main(int argc, char **argv)
{
	...
	unsigned long c1 = 0;
	test_hibenchmarks_internal();
	for(i = 0; i < max ; i++)
		c1 += test_hibenchmarks_internal();
	...
	printf("hibenchmarks internal: completed in %lu cycles, %lu cycles per read, %0.2f %%.\n", c1, c1 / max, (float)c1 * 100.0 / (float)c1);

...
// ----------------------------------------------------------------------------
// send the result to hibenchmarks
...
void detect_veth_interfaces(pid_t pid) {
    ...
    host = read_proc_net_dev(hibenchmarks_configured_host_prefix);
    if(!host) {
        errno = 0;
    ...
    if(switch_namespace(hibenchmarks_configured_host_prefix, pid)) {
        errno = 0;
...
int main(int argc, char **argv) {
    ...
    // ------------------------------------------------------------------------
    // make sure HIBENCHMARKS_HOST_PREFIX is safe

    hibenchmarks_configured_host_prefix = getenv("HIBENCHMARKS_HOST_PREFIX");
    if(verify_hibenchmarks_host_prefix() == -1) exit(1);

    if(hibenchmarks_configured_host_prefix[0] != '\0' && verify_path(hibenchmarks_configured_host_prefix) == -1)
        fatal("invalid HIBENCHMARKS_HOST_PREFIX '%s'", hibenchmarks_configured_host_prefix);

    // ------------------------------------------------------------------------
    // build a safe environment for our script

    // the first environment variable is a fixed PATH=
    snprintfz(environment_variable2, sizeof(environment_variable2) - 1, "HIBENCHMARKS_HOST_PREFIX=%s", hibenchmarks_configured_host_prefix);

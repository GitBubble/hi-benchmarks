...
# https://github.com/firehol/hibenchmarks/issues/474#issuecomment-317866709
...
proc_pid_fdinfo_iff() {
    ...
    set_source "fdinfo"
    grep ^iff:.* "${HIBENCHMARKS_HOST_PREFIX}/proc/${p}/fdinfo"/* 2>/dev/null | cut -f 2
}
...
# print the interfaces found, in the format hibenchmarks expects them
found=0
for x in "${!devs[@]}"
do
    found=$((found + 1))
...
# let hibenchmarks know if we found any
[ ${found} -eq 0 ] && exit 1
exit 0

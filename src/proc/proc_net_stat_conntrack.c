...
int do_proc_net_stat_conntrack(int update_every, usec_t dt) {
    ...
    if(unlikely(do_sockets == -1)) {
        ...
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/net/stat/nf_conntrack");
        nf_conntrack_filename = config_get("plugin:proc:/proc/net/stat/nf_conntrack", "filename to monitor", filename);

        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/sys/net/netfilter/nf_conntrack_max");
        ...
        do_sockets = 1;
        if(!read_full) {
            snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/sys/net/netfilter/nf_conntrack_count");
    ...
    if(do_sockets) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_new) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/nf_conntrack"
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER + 1
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_changes) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/nf_conntrack"
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER + 2
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if(do_expect) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/nf_conntrack"
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER + 3
                    , update_every
                    , RRDSET_TYPE_LINE
            );
            rrdset_flag_set(st, RRDSET_FLAG_DETAIL);
    ...
    if(do_search) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/nf_conntrack"
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER + 10
                    , update_every
                    , RRDSET_TYPE_LINE
            );
            rrdset_flag_set(st, RRDSET_FLAG_DETAIL);
    ...
    if(do_errors) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/nf_conntrack"
                    , HIBENCHMARKS_CHART_PRIO_NETFILTER + 5
                    , update_every
                    , RRDSET_TYPE_LINE
            );
            rrdset_flag_set(st, RRDSET_FLAG_DETAIL);

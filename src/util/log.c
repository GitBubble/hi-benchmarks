...
static hibenchmarks_mutex_t log_mutex = HIBENCHMARKS_MUTEX_INITIALIZER;
static inline void log_lock() {
    hibenchmarks_mutex_lock(&log_mutex);
}
static inline void log_unlock() {
    hibenchmarks_mutex_unlock(&log_mutex);
}
...
void debug_int( const char *file, const char *function, const unsigned long line, const char *fmt, ... ) {
    ...
    va_start( args, fmt );
    printf("%s: %s DEBUG : %s : (%04lu@%-10.10s:%-15.15s): ", date, program_name, hibenchmarks_thread_tag(), line, file, function);
    vprintf(fmt, args);
    va_end( args );
    putchar('\n');
...
void info_int( const char *file, const char *function, const unsigned long line, const char *fmt, ... )
{
    ...
    va_start( args, fmt );
    if(debug_flags) fprintf(stderr, "%s: %s INFO  : %s : (%04lu@%-10.10s:%-15.15s): ", date, program_name, hibenchmarks_thread_tag(), line, file, function);
    else            fprintf(stderr, "%s: %s INFO  : %s : ", date, program_name, hibenchmarks_thread_tag());
...
void error_int( const char *prefix, const char *file, const char *function, const unsigned long line, const char *fmt, ... ) {
    ...
    va_start( args, fmt );
    if(debug_flags) fprintf(stderr, "%s: %s %-5.5s : %s : (%04lu@%-10.10s:%-15.15s): ", date, program_name, prefix, hibenchmarks_thread_tag(), line, file, function);
    else            fprintf(stderr, "%s: %s %-5.5s : %s : ", date, program_name, prefix, hibenchmarks_thread_tag());
    vfprintf( stderr, fmt, args );
    va_end( args );
...
void fatal_int( const char *file, const char *function, const unsigned long line, const char *fmt, ... ) {
    ...
    va_start( args, fmt );
    if(debug_flags) fprintf(stderr, "%s: %s FATAL : %s : (%04lu@%-10.10s:%-15.15s): ", date, program_name, hibenchmarks_thread_tag(), line, file, function);
    else            fprintf(stderr, "%s: %s FATAL : %s :", date, program_name, hibenchmarks_thread_tag());
    vfprintf( stderr, fmt, args );
    va_end( args );
    ...
    hibenchmarks_cleanup_and_exit(1);
}
...
void log_access( const char *fmt, ... ) {
    ...
    if(stdaccess) {
        static hibenchmarks_mutex_t access_mutex = HIBENCHMARKS_MUTEX_INITIALIZER;

        if(web_server_is_multithreaded)
            hibenchmarks_mutex_lock(&access_mutex);
        ...
        if(web_server_is_multithreaded)
            hibenchmarks_mutex_unlock(&access_mutex);
    }
}

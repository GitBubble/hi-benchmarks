...
int do_proc_net_stat_synproxy(int update_every, usec_t dt) {
    ...
    if(unlikely(!ff)) {
        ...
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/net/stat/synproxy");
    ...
    if((do_entries == CONFIG_BOOLEAN_AUTO && events) || do_entries == CONFIG_BOOLEAN_YES) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/synproxy"
                    , HIBENCHMARKS_CHART_PRIO_SYNPROXY + 4
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if((do_syns == CONFIG_BOOLEAN_AUTO && events) || do_syns == CONFIG_BOOLEAN_YES) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/synproxy"
                    , HIBENCHMARKS_CHART_PRIO_SYNPROXY + 1
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if((do_reopened == CONFIG_BOOLEAN_AUTO && events) || do_reopened == CONFIG_BOOLEAN_YES) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/synproxy"
                    , HIBENCHMARKS_CHART_PRIO_SYNPROXY + 3
                    , update_every
                    , RRDSET_TYPE_LINE
            );
    ...
    if((do_cookies == CONFIG_BOOLEAN_AUTO && events) || do_cookies == CONFIG_BOOLEAN_YES) {
        ...
        if(unlikely(!st)) {
            st = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "net/stat/synproxy"
                    , HIBENCHMARKS_CHART_PRIO_SYNPROXY + 2
                    , update_every
                    , RRDSET_TYPE_LINE
            );

...
**Requirements:**
Verify that user hibenchmarks can execute `chronyc tracking`. If necessary, update `/etc/chrony.conf`, `cmdallow`.
...
Directory is also prefixed with `HIBENCHMARKS_HOST_PREFIX` if specified.
...
**Requirement:**
Dovecot unix socket with R/W permissions for user hibenchmarks or dovecot with configured TCP/IP socket.
...
**Requirements:**
 * fail2ban.log file MUST BE readable by hibenchmarks (A good idea is to add  **create 0640 root hibenchmarks** to fail2ban conf at logrotate.d)
...
`go_expvar` produces charts for Go runtime memory statistics and optionally any number of custom charts. Please see the [wiki page](https://github.com/firehol/hibenchmarks/wiki/Monitoring-Go-Applications) for more info.
...
Please see the [wiki page](https://github.com/firehol/hibenchmarks/wiki/Monitoring-Go-Applications#using-hibenchmarks-go_expvar-module) for detailed info about module configuration.
...
**Requirement:**
Socket MUST be readable AND writable by hibenchmarks user.
 ...
 * A system/service/firewall might block hibenchmarks's access if a portscan or
   similar is detected.
...
**Requirements:**
 * dhcpd leases file MUST BE readable by hibenchmarks
...
This module needs no configuration.  Just make sure the hibenchmarks user
...
`/etc/hibenchmarks/python.d.conf` before it will run.
 ...
 * Make sure HIBENCHMARKS USER CAN READ openvpn-status.log
...
sudo and assumes that it is configured such that the `hibenchmarks` user can
execute smbstatus as root without password.

For example:

    hibenchmarks ALL=(ALL)       NOPASSWD: /usr/bin/smbstatus -P
...
For detailed configuration information please read [`sensors.conf`](https://github.com/firehol/hibenchmarks/blob/master/conf.d/python.d/sensors.conf) file.
...
We are tracking such cases in issue [#827](https://github.com/firehol/hibenchmarks/issues/827).
...
Please see the [Monitoring Java Spring Boot Applications](https://github.com/firehol/hibenchmarks/wiki/Monitoring-Java-Spring-Boot-Applications) page for detailed info about module configuration.
...
For detailed configuration information please read [`w1sensor.conf`](https://github.com/firehol/hibenchmarks/blob/master/conf.d/python.d/w1sensor.conf) file.
...
11. **All Time Unique Client IPs** unique ips/s (unique client IPs since the last restart of hibenchmarks)

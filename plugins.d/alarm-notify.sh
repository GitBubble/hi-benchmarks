...
#
# Script to send alarm notifications for hibenchmarks
#
# Features:
...
debug=${HIBENCHMARKS_ALARM_NOTIFY_DEBUG-0}
debug() {
    [ "${debug}" = "1" ] && log DEBUG "${@}"
}
...
docurl() {
    ...
    if [ "${debug}" = "1" ]
        ...
        local out=$(mktemp /tmp/hibenchmarks-health-alarm-notify-XXXXXXXX)
...
[ -z "${HIBENCHMARKS_CONFIG_DIR}"   ] && HIBENCHMARKS_CONFIG_DIR="$(dirname "${0}")/../../../../etc/hibenchmarks"
[ -z "${HIBENCHMARKS_CACHE_DIR}"    ] && HIBENCHMARKS_CACHE_DIR="$(dirname "${0}")/../../../../var/cache/hibenchmarks"
[ -z "${HIBENCHMARKS_REGISTRY_URL}" ] && HIBENCHMARKS_REGISTRY_URL="https://registry.my-hibenchmarks.io"
...
name="${7}"                # the name of the alarm, as given in hibenchmarks health.d entries
...
# -----------------------------------------------------------------------------
# find a suitable hostname to use, if hibenchmarks did not supply a hostname
...
images_base_url="https://registry.my-hibenchmarks.io"
...
if [ -f "${HIBENCHMARKS_CONFIG_DIR}/health_alarm_notify.conf" ]
    then
    source "${HIBENCHMARKS_CONFIG_DIR}/health_alarm_notify.conf"
else
    error "Cannot find file ${HIBENCHMARKS_CONFIG_DIR}/health_alarm_notify.conf. Using internal defaults."
fi
...
filter_recipient_by_criticality() {
    ...
    [ ! -d "${HIBENCHMARKS_CACHE_DIR}/alarm-notify/${method}/${r}" ] && \
        mkdir -p "${HIBENCHMARKS_CACHE_DIR}/alarm-notify/${method}/${r}"

    case "${status}" in
        CRITICAL)
            # make sure he will get future notifications for this alarm too
            touch "${HIBENCHMARKS_CACHE_DIR}/alarm-notify/${method}/${r}/${alarm_id}"
            debug "SEVERITY FILTERING for ${x} VIA ${method}: ALLOW: the alarm is CRITICAL (will now receive next status change)"
            return 0
            ;;

        WARNING)
            if [ -f "${HIBENCHMARKS_CACHE_DIR}/alarm-notify/${method}/${r}/${alarm_id}" ]
            then
        ...
        *)
            ...
            then
                ...
                rm "${HIBENCHMARKS_CACHE_DIR}/alarm-notify/${method}/${r}/${alarm_id}"
...
# hibenchmarks may call us with multiple roles, and roles may have multiple but
    ...
    if [ "${SEND_PUSHOVER}" = "YES" -a ! -z "${apptoken}" -a ! -z "${usertokens}" -a ! -z "${title}" -a ! -z "${message}" ]
        ...
        do
            httpcode=$(docurl \
                ...
                --form-string "url=${url}" \
                --form-string "url_title=Open hibenchmarks dashboard to view the alarm" \
...
goto_url="${HIBENCHMARKS_REGISTRY_URL}/goto-host-from-alarm.html?host=${url_host}&chart=${url_chart}&family=${url_family}&alarm=${url_name}&alarm_unique_id=${unique_id}&alarm_id=${alarm_id}&alarm_event_id=${event_id}"
...
<a href=\\\"${goto_url}\\\">View hibenchmarks dashboard</a> \
(source of alarm ${src}) \
"
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="background: #fff; border: 1px solid #e9e9e9;">
                    ...
                    <tr>
                        <td bgcolor="#eee" style="padding: 5px 20px 5px 20px; background-color: #eee;">
                            <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 20px; color: #777; font-weight: bold;">hibenchmarks notification</div>
                        </td>
                    </tr>
                    <tr>
                                ...
                                <table width="100%" cellpadding="0" cellspacing="0" style="max-width:700px">
                                    ...
                                    <tr style="text-align: center; margin: 0; padding: 0;">
                                        <td style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 12px; vertical-align: top; margin:0; padding: 20px 0 0 0; color: #666666; border-top: 1px solid #f0f0f0;" align="center" valign="bottom">Sent by
                                            <a href="https://myhibenchmarks.io/" target="_blank">hibenchmarks</a>, the real-time performance and health monitoring, on <code>${this_host}</code>.
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>

# -*- coding: utf-8 -*-
# Description: elastic search node stats hibenchmarks python.d module
# Author: l2isbad
...
class Service(UrlService):
    ...
    def _get_cluster_health(self, queue, url):
        ...
        data = loads(raw_data)
        to_hibenchmarks = fetch_data_(raw_data=data,
                                 metrics=HEALTH_STATS)

        to_hibenchmarks.update({'status_green': 0, 'status_red': 0, 'status_yellow': 0,
                           'status_foo1': 0, 'status_foo2': 0, 'status_foo3': 0})
        current_status = 'status_' + data['status']
        to_hibenchmarks[current_status] = 1

        return queue.put(to_hibenchmarks)
    ...
    def _get_node_stats(self, queue, url):
        ...
        to_hibenchmarks = fetch_data_(raw_data=data['nodes'][node],
                                 metrics=NODE_STATS)

        # Search, index, flush, fetch performance latency
        for key in LATENCY:
            try:
                to_hibenchmarks[key] = self.find_avg(total=to_hibenchmarks[LATENCY[key]['total']],
                                                spent_time=to_hibenchmarks[LATENCY[key]['spent_time']],
                                                key=key)
            except KeyError:
                continue
        if 'process_open_file_descriptors' in to_hibenchmarks and 'process_max_file_descriptors' in to_hibenchmarks:
            to_hibenchmarks['file_descriptors_used'] = round(float(to_hibenchmarks['process_open_file_descriptors'])
                                                        / to_hibenchmarks['process_max_file_descriptors'] * 1000)

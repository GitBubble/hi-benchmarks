# -*- coding: utf-8 -*-
# Description: MySQL hibenchmarks python.d module
        ...
        if 'global_status' in raw_data:
            ...
            for key in GLOBAL_STATS:
                if key in global_status:
                    to_hibenchmarks[key] = global_status[key]
            if 'Threads_created' in to_hibenchmarks and 'Connections' in to_hibenchmarks:
                to_hibenchmarks['Thread_cache_misses'] = round(int(to_hibenchmarks['Threads_created'])
                                                          / float(to_hibenchmarks['Connections']) * 10000)
            ...
            if raw_data['slave_status'][0]:
                ...
                for key, func in SLAVE_STATS:
                    if key in slave_raw_data:
                        to_hibenchmarks[key] = func(slave_raw_data[key])
            else:
                self.queries.pop('slave_status')

        return to_hibenchmarks or None

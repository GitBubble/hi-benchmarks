...
void *freebsd_main(void *ptr) {
    hibenchmarks_thread_cleanup_push(freebsd_main_cleanup, ptr);

    int vdo_cpu_hibenchmarks = config_get_boolean("plugin:freebsd", "hibenchmarks server resources", 1);
    ...
    while(!hibenchmarks_exit) {
        ...
        if(vdo_cpu_hibenchmarks) {
            ...
            if(unlikely(!st)) {
                st = rrdset_find_bytype_localhost("hibenchmarks", "plugin_freebsd_modules");

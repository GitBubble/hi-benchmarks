    ...
    * Banned IPs for every jail (since the last restart of hibenchmarks)
        ...
        for row in raw:
            ...
            if match:
                ...
                if jail in self.jails_list:
                    if action == 'B':
                        self.to_hibenchmarks[jail] += 1
                        if address_not_in_jail(self.banned_ips[jail], ipaddr, self.to_hibenchmarks[jail + '_in_jail']):
                            self.to_hibenchmarks[jail + '_in_jail'] += 1
                    else:
    ...
    def check(self):
        ...
        if not (is_accessible(self.log_path, R_OK) and getsize(self.log_path) != 0):
            ...
            return False
        self.jails_list, self.to_hibenchmarks, self.banned_ips = self.jails_auto_detection_()
    ...
    def jails_auto_detection_(self):
        ...
        * to_hibenchmarks - dict ({'ssh': 0, 'ssh_in_jail': 0, ...})
        * banned_ips - here will be stored all the banned ips ({'ssh': ['1.2.3.4', '5.6.7.8', ...], ...})
        ...
        to_hibenchmarks = dict([(jail, 0) for jail in jails_list])
        to_hibenchmarks.update(dict([(jail + '_in_jail', 0) for jail in jails_list]))
        banned_ips = dict([(jail, list()) for jail in jails_list])

        return jails_list, to_hibenchmarks, banned_ips
...
def create_definitions_(jails_list):
    ...
    definitions = {
        'jails_bans': {'options': [None, 'Jails Ban Statistics', 'bans/s', 'bans', 'jail.bans', 'line'],
                       'lines': []},
        'jails_in_jail': {'options': [None, 'Banned IPs (since the last restart of hibenchmarks)', 'IPs',

...
static inline char *get_disk_name(unsigned long major, unsigned long minor, char *disk) {
    ...
    hibenchmarks_fix_chart_name(result);
...
int do_proc_diskstats(int update_every, usec_t dt) {
    ...
    if(unlikely(!globals_initialized)) {
        ...
        snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/block/%s");
        path_to_sys_block_device = config_get(CONFIG_SECTION_DISKSTATS, "path to get block device", buffer);

        snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/block/%s/bcache");
        path_to_sys_block_device_bcache = config_get(CONFIG_SECTION_DISKSTATS, "path to get block device bcache", buffer);

        snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/devices/virtual/block/%s");
        path_to_sys_devices_virtual_block_device = config_get(CONFIG_SECTION_DISKSTATS, "path to get virtual block device", buffer);

        snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/dev/block/%lu:%lu/%s");
        path_to_sys_dev_block_major_minor_string = config_get(CONFIG_SECTION_DISKSTATS, "path to get block device infos", buffer);

        //snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/block/%s/queue/hw_sector_size");
        //path_to_get_hw_sector_size = config_get(CONFIG_SECTION_DISKSTATS, "path to get h/w sector size", buffer);

        //snprintfz(buffer, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/dev/block/%lu:%lu/subsystem/%s/../queue/hw_sector_size");
        ...
        snprintfz(buffer, FILENAME_MAX, "%s/dev/mapper", hibenchmarks_configured_host_prefix);
        path_to_device_mapper = config_get(CONFIG_SECTION_DISKSTATS, "path to device mapper", buffer);

        snprintfz(buffer, FILENAME_MAX, "%s/dev/disk/by-label", hibenchmarks_configured_host_prefix);
        path_to_device_label = config_get(CONFIG_SECTION_DISKSTATS, "path to /dev/disk/by-label", buffer);

        snprintfz(buffer, FILENAME_MAX, "%s/dev/disk/by-id", hibenchmarks_configured_host_prefix);
    ...
    if(unlikely(!ff)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/proc/diskstats");

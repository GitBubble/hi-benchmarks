...
CONFIG="${HIBENCHMARKS_CONFIG_DIR}/cgroups-names.conf"
CGROUP="${1}"
NAME=
...
if [ -z "${NAME}" ]
    ...
    if [[ "${CGROUP}" =~ ^.*docker[-_/\.][a-fA-F0-9]+[-_\.]?.*$ ]]
        ...
        if [[ -f $FILENAME && -r $FILENAME ]]
            ...
            error "proxmox config file missing ${FILENAME} or hibenchmarks does not have read access.  Please ensure hibenchmarks is a member of www-data group."
        fi

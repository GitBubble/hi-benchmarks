...
// example configuration in /etc/hibenchmarks/node.d/sma_webbox.conf
/*
{
...
var webbox = {
    ...
    serviceExecute: function(name, hostname, update_every) {
        if(hibenchmarks.options.DEBUG === true) hibenchmarks.debug(this.name + ': ' + name + ': hostname: ' + hostname + ', update_every: ' + update_every);

        var service = hibenchmarks.service({
            name: name,
            request: hibenchmarks.requestFromURL('http://' + hostname + '/rpc'),

...
if [ "$(uname -m)" != "x86_64" ]
	then
	fatal "Static binary versions of hibenchmarks are available only for 64bit Intel/AMD CPUs (x86_64), but yours is: $(uname -m)."
fi

if [ "$(uname -s)" != "Linux" ]
	then
	fatal "Static binary versions of hibenchmarks are available only for Linux, but this system is $(uname -s)"
fi
...
if [ ! -z "${curl}" -a -x "${curl}" ]
then
    LATEST="$(run ${curl} "${BASE}/hibenchmarks-latest.gz.run")"
elif [ ! -z "${wget}" -a -x "${wget}" ]
then
    LATEST="$(run ${wget} -O - "${BASE}/hibenchmarks-latest.gz.run")"
else
...
if [ -z "${LATEST}" ]
	then
	fatal "Cannot find the latest static binary version of hibenchmarks."
fi

# ---------------------------------------------------------------------------------------------------------------------

progress "Downloading static hibenchmarks binary: ${LATEST}"
...
if [ ${ret} -ne 0 -o ! -s "/tmp/${LATEST}" ]
	then
	fatal "Failed to download the latest static binary version of hibenchmarks."
fi

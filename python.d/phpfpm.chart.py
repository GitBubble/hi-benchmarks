# -*- coding: utf-8 -*-
# Description: PHP-FPM hibenchmarks python.d module
...
class Service(UrlService):
    ...
    def _get_data(self):
        ...
        to_hibenchmarks = fetch_data_(raw_data=raw_json, metrics_list=POOL_INFO)

        # Per Process Info: duration, cpu and memory charts (min, max, avg)
        if self.json_full:
            p_info = dict()
            to_hibenchmarks.update(self.if_all_processes_running)  # If all processes are in running state
            ...
            if p_info:
                for new_name in PER_PROCESS_INFO:
                    for name, func in CALC:
                        to_hibenchmarks[name + new_name[1]] = func([p_info[k] for k in p_info if new_name[1] in k])

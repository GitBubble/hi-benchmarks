...
// hibenchmarks snmp module
// This program will connect to one or more SNMP Agents
//

// example configuration in /etc/hibenchmarks/node.d/snmp.conf
/*
{
...
hibenchmarks.processors.snmp = {
    ...
    prepare: function(service) {
        var __DEBUG = hibenchmarks.options.DEBUG;
        ...
        if(typeof service.snmp_oids === 'undefined' || service.snmp_oids === null || service.snmp_oids.length === 0) {
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(service.module.name + ': ' + service.name + ': preparing ' + this.name + ' OIDs');
            ...
            while(chart_keys_len--) {
                ...
                if(__DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': indexing ' + this.name + ' chart: ' + c);
                ...
                while(dim_keys_len--) {
                    ...
                    if(__DEBUG === true)
                        hibenchmarks.debug(service.module.name + ': ' + service.name + ': indexing ' + this.name + ' chart: ' + c + ', dimension: ' + d + ', OID: ' + oid + ", OID name: " + oidname);
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(service.module.name + ': ' + service.name + ': indexed ' + this.name + ' OIDs: ' + hibenchmarks.stringify(service.snmp_oids_index));

            // now create the array of OIDs needed by net-snmp
            service.snmp_oids = Object.keys(service.snmp_oids_index);

            if(__DEBUG === true)
                hibenchmarks.debug(service.module.name + ': ' + service.name + ': final list of ' + this.name + ' OIDs: ' + hibenchmarks.stringify(service.snmp_oids));
    ...
    getdata: function(service, index, ok, failed, callback) {
        ...
        if(__DEBUG === true)
            hibenchmarks.debug(service.module.name + ': ' + service.name + ': making ' + slice.length + ' entries request, max is: ' + service.request.max_request_size);

        service.snmp_session.get(slice, function(error, varbinds) {
            if(error) {
                service.error('Received error = ' + hibenchmarks.stringify(error) + ' varbinds = ' + hibenchmarks.stringify(varbinds));
                ...
                if(__DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': got valid ' + service.module.name + ' response: ' + hibenchmarks.stringify(varbinds));
                ...
                for(var i = 0; i < varbinds_len ; i++) {
                    ...
                    if(net_snmp.isVarbindError(varbinds[i])) {
                        if(__DEBUG === true)
                            hibenchmarks.debug(service.module.name + ': ' + service.name + ': failed ' + service.module.name + ' get for OIDs ' + varbinds[i].oid);
                        ...
                        switch(varbinds[i].type) {
                            case net_snmp.ObjectType.OctetString:
                                if(service.snmp_oids_index[varbinds[i].oid].type !== 'title')
                                    ...
                                    if(__DEBUG === true) hibenchmarks.debug(service.module.name + ': ' + service.name + ': found ' + service.module.name + ' value of OIDs ' + varbinds[i].oid + ", ObjectType " + net_snmp.ObjectType[varbinds[i].type] + " (" + hibenchmarks.stringify(varbinds[i].type) + "), typeof(" + typeof(varbinds[i].value) + "), in JSON: " + hibenchmarks.stringify(varbinds[i].value) + ", value = " + value.toString() + " (parsed as float in string)");
                                else
                                    // just use the string
                ...
                if(__DEBUG === true)
                    hibenchmarks.debug(service.module.name + ': ' + service.name + ': finished ' + service.module.name + ' with ' + ok + ' successful and ' + failed + ' failed values');
            }
    ...
    process: function(service, callback) {
        ...
        if(typeof service.snmp_session === 'undefined' || service.snmp_session === null) {
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(service.module.name + ': ' + service.name + ': opening ' + this.name + ' session on ' + service.request.hostname + ' community ' + service.request.community + ' options ' + hibenchmarks.stringify(service.request.options));
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(service.module.name + ': ' + service.name + ': got ' + this.name + ' session: ' + hibenchmarks.stringify(service.snmp_session));

            // if we later need traps, this is how to do it:
            //service.snmp_session.trap(net_snmp.TrapType.LinkDown, function(error) {
            //  if(error) console.error('trap error: ' + hibenchmarks.stringify(error));
            //});
        }
...
var snmp = {
    ...
    serviceExecute: function(conf) {
        ...
        if(__DEBUG === true)
            hibenchmarks.debug(this.name + ': snmp hostname: ' + conf.hostname + ', update_every: ' + conf.update_every);
        ...
        var service = hibenchmarks.service({
            ...
            module: this,
            processor: hibenchmarks.processors.snmp
        });
        ...
        for( var i = 0; i < chart_keys_len ; i++ ) {
            ...
            if(__DEBUG === true)
                hibenchmarks.debug(this.name + ': snmp hostname: ' + conf.hostname + ', examining chart: ' + c);

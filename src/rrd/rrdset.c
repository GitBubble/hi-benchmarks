...
void __rrdset_check_rdlock(RRDSET *st, const char *file, const char *function, const unsigned long line) {
    ...
    int ret = hibenchmarks_rwlock_trywrlock(&st->rrdset_rwlock);
    if(ret == 0)
...
void __rrdset_check_wrlock(RRDSET *st, const char *file, const char *function, const unsigned long line) {
    ...
    int ret = hibenchmarks_rwlock_tryrdlock(&st->rrdset_rwlock);
    if(ret == 0)
...
void rrdset_free(RRDSET *st) {
    ...
    hibenchmarks_rwlock_destroy(&st->rrdset_rwlock);
...
RRDSET *rrdset_create_custom(
    ...
    if(memory_mode == RRD_MEMORY_MODE_SAVE || memory_mode == RRD_MEMORY_MODE_MAP || memory_mode == RRD_MEMORY_MODE_RAM) {
        ...
        if(st) {
            ...
            memset(&st->rrdset_rwlock, 0, sizeof(hibenchmarks_rwlock_t));
    ...
    hibenchmarks_rwlock_init(&st->rrdset_rwlock);
...
void rrdset_done(RRDSET *st) {
    if(unlikely(hibenchmarks_exit)) return;

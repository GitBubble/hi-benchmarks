...
int do_sys_kernel_mm_ksm(int update_every, usec_t dt) {
    ...
    if(unlikely(!ff_pages_shared)) {
        snprintfz(values[PAGES_SHARED].filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/kernel/mm/ksm/pages_shared");
    ...
    if(unlikely(!ff_pages_sharing)) {
        snprintfz(values[PAGES_SHARING].filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/kernel/mm/ksm/pages_sharing");
    ...
    if(unlikely(!ff_pages_unshared)) {
        snprintfz(values[PAGES_UNSHARED].filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/kernel/mm/ksm/pages_unshared");
    ...
    if(unlikely(!ff_pages_volatile)) {
        snprintfz(values[PAGES_VOLATILE].filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/kernel/mm/ksm/pages_volatile");
    ...
    //    snprintfz(values[PAGES_TO_SCAN].filename, FILENAME_MAX, "%s%s", hibenchmarks_configured_host_prefix, "/sys/kernel/mm/ksm/pages_to_scan");
...

    {
        ...
        if (unlikely(!st_mem_ksm)) {
            st_mem_ksm = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "/sys/kernel/mm/ksm"
                    , HIBENCHMARKS_CHART_PRIO_MEM_KSM
                    , update_every
                    , RRDSET_TYPE_AREA
            );
...

    {
        ...
        if (unlikely(!st_mem_ksm_savings)) {
            st_mem_ksm_savings = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "/sys/kernel/mm/ksm"
                    , HIBENCHMARKS_CHART_PRIO_MEM_KSM + 1
                    , update_every
                    , RRDSET_TYPE_AREA
            );
...

    {
        ...
        if (unlikely(!st_mem_ksm_ratios)) {
            st_mem_ksm_ratios = rrdset_create_localhost(
                    ...
                    , "proc"
                    , "/sys/kernel/mm/ksm"
                    , HIBENCHMARKS_CHART_PRIO_MEM_KSM + 2
                    , update_every
                    , RRDSET_TYPE_LINE
            );

...
int accept_socket(int fd, int flags, char *client_ip, size_t ipsize, char *client_port, size_t portsize, SIMPLE_PATTERN *access_list) {
    ...
    if (likely(nfd >= 0)) {
        ...
        error("hibenchmarks has been compiled with the assumption that the system has the accept4() call, but it is not here. Recompile hibenchmarks like this: ./configure --disable-accept4 ...");
#endif
...
void poll_events(LISTEN_SOCKETS *sockets
    ...
    hibenchmarks_thread_cleanup_push(poll_events_cleanup, &p);

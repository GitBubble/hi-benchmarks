...
// ----------------------------------------------------------------------------
// the rates we are going to send to hibenchmarks will have this detail a value of:
//  - 1 will send just integer parts to hibenchmarks
...
// will be changed to getenv(HIBENCHMARKS_CONFIG_DIR) if it exists
...
struct target {
    ...
    int exposed;            // if set, we have sent this to hibenchmarks
    ...
    struct target *target;  // the one that will be reported to hibenchmarks
    struct target *next;
};
...
static struct target *get_users_target(uid_t uid) {
    ...
    hibenchmarks_fix_chart_name(w->name);
            ...
            if(unlikely(debug_enabled || !(p->log_thrown & log))) {
                ...
                switch(log) {
                    case PID_LOG_IO:
                        ...
                        #else
                        error("Cannot process %s/proc/%d/io (command '%s')", hibenchmarks_configured_host_prefix, p->pid, p->comm);
                        #endif
                        break;
                    ...
                    case PID_LOG_STATUS:
                        ...
                        #else
                        error("Cannot process %s/proc/%d/status (command '%s')", hibenchmarks_configured_host_prefix, p->pid, p->comm);
                        #endif
                        break;
                    ...
                    case PID_LOG_CMDLINE:
                        ...
                        #else
                        error("Cannot process %s/proc/%d/cmdline (command '%s')", hibenchmarks_configured_host_prefix, p->pid, p->comm);
                        #endif
                        break;
                    ...
                    case PID_LOG_FDS:
                        ...
                        #else
                        error("Cannot process entries in %s/proc/%d/fd (command '%s')", hibenchmarks_configured_host_prefix, p->pid, p->comm);
                        #endif
                        break;
...
static inline int read_proc_pid_cmdline(struct pid_stat *p) {
    ...
    if(unlikely(!p->cmdline_filename)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s/proc/%d/cmdline", hibenchmarks_configured_host_prefix, p->pid);
...
static inline int read_proc_pid_status(struct pid_stat *p, void *ptr) {
    ...
    if(unlikely(!p->status_filename)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s/proc/%d/status", hibenchmarks_configured_host_prefix, p->pid);
...
static inline int read_proc_pid_stat(struct pid_stat *p, void *ptr) {
    ...
    if(unlikely(!p->stat_filename)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s/proc/%d/stat", hibenchmarks_configured_host_prefix, p->pid);
...
static inline int read_proc_pid_io(struct pid_stat *p, void *ptr) {
    ...
    if(unlikely(!p->io_filename)) {
        char filename[FILENAME_MAX + 1];
        snprintfz(filename, FILENAME_MAX, "%s/proc/%d/io", hibenchmarks_configured_host_prefix, p->pid);
...
static inline int read_proc_stat() {
    ...
    if(unlikely(!ff)) {
        snprintfz(filename, FILENAME_MAX, "%s/proc/stat", hibenchmarks_configured_host_prefix);
...
static inline int read_pid_file_descriptors(struct pid_stat *p, void *ptr) {
    ...
    if(unlikely(!p->fds_dirname)) {
        ...
        snprintfz(dirname, FILENAME_MAX, "%s/proc/%d/fd", hibenchmarks_configured_host_prefix, p->pid);
    ...
    while((de = readdir(fds))) {
        ...
        if(unlikely(p->fds[fdid] == 0)) {
            ...
            sprintf(fdname, "%s/proc/%d/fd/%s", hibenchmarks_configured_host_prefix, p->pid, de->d_name);
...
static int collect_data_for_all_processes(void) {
    ...
    snprintfz(dirname, FILENAME_MAX, "%s/proc", hibenchmarks_configured_host_prefix);
...
static void calculate_hibenchmarks_statistics(void) {
...
void send_resource_usage_to_hibenchmarks(usec_t dt) {
    ...
    if(unlikely(!created_charts)) {
        ...
        fprintf(stdout,
                "CHART hibenchmarks.apps_cpu '' 'Apps Plugin CPU' 'milliseconds/s' apps.plugin hibenchmarks.apps_cpu stacked 140000 %1$d\n"
                "DIMENSION user '' incremental 1 1000\n"
                "DIMENSION system '' incremental 1 1000\n"
                "CHART hibenchmarks.apps_sizes '' 'Apps Plugin Files' 'files/s' apps.plugin hibenchmarks.apps_sizes line 140001 %1$d\n"
...
#if (ALL_PIDS_ARE_READ_INSTANTLY == 0)
        fprintf(stdout,
                "CHART hibenchmarks.apps_fix '' 'Apps Plugin Normalization Ratios' 'percentage' apps.plugin hibenchmarks.apps_fix line 140002 %1$d\n"
        ...
        if(include_exited_childs)
            fprintf(stdout,
                    "CHART hibenchmarks.apps_children_fix '' 'Apps Plugin Exited Children Normalization Ratios' 'percentage' apps.plugin hibenchmarks.apps_children_fix line 140003 %1$d\n"
    ...
    fprintf(stdout,
        "BEGIN hibenchmarks.apps_cpu %llu\n"
        "SET user = %llu\n"
        "SET system = %llu\n"
        "END\n"
        "BEGIN hibenchmarks.apps_sizes %llu\n"
        "SET calls = %zu\n"
        "SET files = %zu\n"
        "SET pids = %zu\n"
        "SET fds = %d\n"
...
#if (ALL_PIDS_ARE_READ_INSTANTLY == 0)
    fprintf(stdout,
            "BEGIN hibenchmarks.apps_fix %llu\n"
            "SET utime = %u\n"
            "SET stime = %u\n"
            "SET gtime = %u\n"
            "SET minflt = %u\n"
    ...
    if(include_exited_childs)
        fprintf(stdout,
            "BEGIN hibenchmarks.apps_children_fix %llu\n"
            "SET cutime = %u\n"
            "SET cstime = %u\n"
            "SET cgtime = %u\n"
            "SET cminflt = %u\n"
...
static void normalize_utilization(struct target *root) {
    ...
    // which is quite big to do it again (hibenchmarks does it already)
    //
    // a better solution could be to somehow have hibenchmarks
...
static void send_collected_data_to_hibenchmarks(struct target *root, const char *type, usec_t dt) {
    struct target *w;
...
static void send_charts_updates_to_hibenchmarks(struct target *root, const char *type, const char *title)
{
    struct target *w;
    int newly_added = 0;
...
int main(int argc, char **argv) {
    ...
    hibenchmarks_configured_host_prefix = getenv("HIBENCHMARKS_HOST_PREFIX");
    if(verify_hibenchmarks_host_prefix() == -1) exit(1);

    config_dir = getenv("HIBENCHMARKS_CONFIG_DIR");
    if(config_dir == NULL) {
        // info("HIBENCHMARKS_CONFIG_DIR is not passed from hibenchmarks");
        config_dir = CONFIG_DIR;
    }
    // else info("Found HIBENCHMARKS_CONFIG_DIR='%s'", config_dir);
    ...
    if(!check_capabilities() && !am_i_running_as_root() && !check_proc_1_io()) {
        ...
        error("apps.plugin should either run as root (now running with uid %u, euid %u) or have special capabilities. "
                      ...
                      "To enable setuid to root run: sudo chown root:hibenchmarks %s; sudo chmod 4750 %s; "
...
#warning "compiling for profiling"
        ...
        calculate_hibenchmarks_statistics();
        normalize_utilization(apps_groups_root_target);

        send_resource_usage_to_hibenchmarks(dt);

        // this is smart enough to show only newly added apps, when needed
        send_charts_updates_to_hibenchmarks(apps_groups_root_target, "apps", "Apps");

        if(likely(enable_users_charts))
            send_charts_updates_to_hibenchmarks(users_root_target, "users", "Users");

        if(likely(enable_groups_charts))
            send_charts_updates_to_hibenchmarks(groups_root_target, "groups", "User Groups");

        send_collected_data_to_hibenchmarks(apps_groups_root_target, "apps", dt);

        if(likely(enable_users_charts))
            send_collected_data_to_hibenchmarks(users_root_target, "users", dt);

        if(likely(enable_groups_charts))
            send_collected_data_to_hibenchmarks(groups_root_target, "groups", dt);
